...
class ProfileQueryPermissionTest(unittest.TestCase):
    ...
    def exercise(self, container):
        ...
        self.assertEqual('1,3,4',self.sql(container,f"SELECT GROUP_CONCAT(role_id ORDER BY role_id) FROM system_role_menu WHERE menu_id={menu} AND deleted=0"))
        ...
        self.sql(container,script)
        self.assertEqual('3',self.sql(container,f"SELECT COUNT(*) FROM system_role_menu WHERE menu_id={menu} AND deleted=0"))
        ...
        print('PASS: configured grants, unrelated role/package isolation, retired menu preserved, replay, UTF-8 HEX')
