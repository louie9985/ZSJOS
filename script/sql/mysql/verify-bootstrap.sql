          ...
          AND NOT EXISTS (SELECT 1 FROM zsjos_lead_inbox_filter_scheme
            WHERE audience='reviewer' AND deleted=b'0'
              AND (draft_config_json LIKE '%\"key\":\"registrationReview\"%'
                ...
                OR published_config_json LIKE '%\"key\": \"financeReview\"%')),
          'PASS', 'FAIL') AS result;
SELECT 'lifecycle_domain_v033' AS check_name,
       IF(EXISTS (SELECT 1 FROM zsjos_schema_version WHERE version='V033')
          AND (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE()
            AND table_name IN ('zsjos_lead','zsjos_opportunity') AND column_name='active_person_id'
            AND extra LIKE '%STORED GENERATED%')=2
          AND (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE()
            AND table_name IN ('zsjos_lead','zsjos_opportunity') AND index_name='uk_tenant_active_person'
            AND non_unique=0)=4
          AND EXISTS (SELECT 1 FROM system_dict_data WHERE dict_type='zsjos_lead_status'
            AND value='won' AND status=0 AND deleted=b'0')
          AND NOT EXISTS (SELECT 1 FROM system_dict_data WHERE dict_type='zsjos_lead_status'
            AND value='converted' AND status=0 AND deleted=b'0'),
          'PASS', 'FAIL') AS result;
...
SELECT 'module_schema_versions' AS check_name,
       IF((SELECT COUNT(*) FROM zsjos_module_schema_version WHERE module_code='core' AND version IN ('V001','V017','V018','V019','V020','V021','V023','V024','V025','V026','V033'))=11, 'PASS', 'FAIL') AS result;
