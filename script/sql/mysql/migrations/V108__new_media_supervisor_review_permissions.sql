-- V108: supervisor graduation permission for new-media closure flows.
...
FROM system_role r JOIN system_menu m ON m.permission IN ('zsjos:student-ops:graduate')
WHERE r.tenant_id=1 AND r.status=0 AND r.deleted=b'0' AND m.deleted=b'0'
  AND r.code='delivery_manager' AND m.permission IN ('zsjos:student-ops:graduate')
...
VALUES ('V108','New-media supervisor graduation permissions','new-media-supervisor-graduation-permissions-v1')
...
VALUES ('core','V108','New-media supervisor graduation permissions',SHA2('new-media-supervisor-graduation-permissions-v1',256),'baseline',NOW())
ON DUPLICATE KEY UPDATE description=VALUES(description),checksum=VALUES(checksum);
