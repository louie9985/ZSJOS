-- V195: repair delivery-class access after the unified-menu migration.
-- The visible pages are shared; query-managed/query-my remain independent data-scope capabilities.
SET NAMES utf8mb4;
START TRANSACTION;

UPDATE system_menu
SET name='学员管理', visible=b'1', updater='V195', update_time=NOW()
...
SET managed.deleted=b'1', managed.updater='V195', managed.update_time=NOW()
...
SELECT source.role_id,73020,'V195',NOW(),'V195',NOW(),b'0',source.tenant_id
...
SET menu_ids=JSON_ARRAY_APPEND(menu_ids,'$',73020), updater='V195', update_time=NOW()
...
VALUES ('V195','Repair delivery class access',SHA2('V195__delivery_class_access_repair.sql',256),NOW())
...
VALUES ('core','V195','Repair delivery class access',SHA2('V195__delivery_class_access_repair.sql',256),'baseline',NOW())
ON DUPLICATE KEY UPDATE description=VALUES(description),checksum=VALUES(checksum);
