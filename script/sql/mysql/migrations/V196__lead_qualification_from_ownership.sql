-- UTF-8. V196: start lead qualification timing at ownership time and backfill active owned leads.
-- Dependencies: V195 migration chain, V014 qualification columns, V006 ownership timestamps,
-- and the enabled tenant rule.
...
SET NAMES utf8mb4;
DROP PROCEDURE IF EXISTS zsjos_v196_apply;
DELIMITER $$
CREATE PROCEDURE zsjos_v196_apply()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM zsjos_schema_version WHERE version='V195') THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='V196 requires V195';
  END IF;

  DROP TEMPORARY TABLE IF EXISTS zsjos_v196_backfill;
  CREATE TEMPORARY TABLE zsjos_v196_backfill AS
  ...
  UPDATE zsjos_lead l JOIN zsjos_v196_backfill b ON b.id = l.id
  ...
  SELECT 'lead_qualification', 'lead', b.id, 'pending', 'user', b.owner_user_id,
         ...
         JSON_OBJECT('roundNo', b.round_no, 'ruleId', b.rule_id,
             'ruleVersion', b.rule_version, 'timeoutMinutes', b.timeout_minutes),
         CONCAT('lead-qualification:', b.id, ':', b.round_no), 0, 'V196', 'V196', b.tenant_id
    FROM zsjos_v196_backfill b
  ...
  SELECT 'lead_qualification_started', 'lead', b.id, b.owner_user_id, 'submitted', 'submitted',
         ...
         b.started_at, CONCAT('lead-qualification-started:', b.id, ':', b.round_no), 'V196', 'V196', b.tenant_id
    FROM zsjos_v196_backfill b
  ...
  VALUES ('V196','Lead qualification timing from ownership',SHA2('V196__lead_qualification_from_ownership.sql',256),NOW())
  ...
  VALUES ('core','V196','Lead qualification timing from ownership',SHA2('V196__lead_qualification_from_ownership.sql',256),'baseline',NOW())
  ON DUPLICATE KEY UPDATE description=VALUES(description), checksum=VALUES(checksum);
END$$
DELIMITER ;
CALL zsjos_v196_apply();
DROP PROCEDURE IF EXISTS zsjos_v196_apply;
