-- V178: decouple lead submission from post checks and add the specified-sales permission.
-- Dependencies: V177 in both schema-version registries and the existing lead submit page menu 6736.
...
DROP PROCEDURE IF EXISTS `zsjos_v178_apply`;
DELIMITER $$
CREATE PROCEDURE `zsjos_v178_apply`()
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM `zsjos_schema_version` WHERE `version`='V177')
     OR NOT EXISTS (SELECT 1 FROM `zsjos_module_schema_version` WHERE `module_code`='core' AND `version`='V177') THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='V178 requires V177 in both schema-version registries';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM `system_menu` WHERE `id`=6736 AND `permission`='zsjos:lead:submit' AND `deleted`=b'0') THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='V178 requires lead submit menu 6736';
  END IF;
  ...
  VALUES
    (6820,'指定销售','zsjos:lead:submit:specify',3,1,6736,'','','',NULL,0,b'1',b'1',b'1',
     'migration-V178',NOW(),'migration-V178',NOW(),b'0')
  ON DUPLICATE KEY UPDATE
    ...
    `updater`='migration-V178',`update_time`=NOW();

  UPDATE `system_tenant_package`
  SET `menu_ids`=JSON_ARRAY_APPEND(`menu_ids`,'$',6820),`updater`='migration-V178',`update_time`=NOW()
  WHERE `deleted`=b'0' AND JSON_CONTAINS(`menu_ids`,'6736','$') AND NOT JSON_CONTAINS(`menu_ids`,'6820','$');

  INSERT INTO `zsjos_schema_version` (`version`,`description`,`checksum`,`installed_at`)
  VALUES ('V178','Lead submit permission decoupling','V178__lead_submit_permission_decoupling.sql',NOW())
  ...
  VALUES ('core','V178','Lead submit permission decoupling',SHA2('V178__lead_submit_permission_decoupling.sql',256),'baseline',NOW())
  ON DUPLICATE KEY UPDATE `description`=VALUES(`description`),`checksum`=VALUES(`checksum`);

  COMMIT;
END$$
DELIMITER ;
CALL `zsjos_v178_apply`();
DROP PROCEDURE IF EXISTS `zsjos_v178_apply`;
