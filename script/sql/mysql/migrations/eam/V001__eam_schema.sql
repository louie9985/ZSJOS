-- V001: install the EAM (Enterprise Asset Management) module schema.
-- Module: eam. Depends on: core (system_users / system_dept exist as identifier targets only).
-- Data scope: creates 9 EAM tables. Inserts no business rows.
-- Repeatability: every statement uses CREATE TABLE IF NOT EXISTS; re-running is a no-op.
-- Rollback limitation: dropping these tables discards all asset ledger data; take a backup first.
