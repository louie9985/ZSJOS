-- V037: unify Customer, Lead, Opportunity, and first-purchase Order lifecycle invariants.
...
-- Execution order: assert Customer relations, normalize states/owners/config, add generated keys/indexes, record V037.
...
DROP PROCEDURE IF EXISTS `zsjos_v037_assert_customer_relations`;
DELIMITER $$
CREATE PROCEDURE `zsjos_v037_assert_customer_relations`()
BEGIN
  IF EXISTS (
    SELECT 1 FROM `zsjos_lead` WHERE `deleted`=b'0'
    GROUP BY `tenant_id`, `person_id` HAVING COUNT(*) > 1
  ) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'V037 blocked: one customer has multiple active leads';
  END IF;
  IF EXISTS (
    SELECT 1
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'V037 blocked: one customer has multiple active opportunities';
  END IF;
END$$
DELIMITER ;
CALL `zsjos_v037_assert_customer_relations`();
DROP PROCEDURE `zsjos_v037_assert_customer_relations`;

UPDATE `zsjos_lead`
SET `status`='valid', `assignment_status`='owned', `updater`='migration-V037', `update_time`=NOW()
...
SET l.`status`='won', l.`assignment_status`='owned', l.`updater`='migration-V037', l.`update_time`=NOW()
...
SET o.`person_id`=l.`person_id`, o.`owner_user_id`=l.`owner_user_id`,
    o.`updater`='migration-V037', o.`update_time`=NOW()
...
SET `status`=1, `updater`='migration-V037', `update_time`=NOW()
...
SELECT 70,'已成交','won','zsjos_lead_status',0,'success','migration-V037',NOW(),'migration-V037',NOW(),b'0'
WHERE NOT EXISTS (
...
SET `draft_config_json`=REPLACE(REPLACE(`draft_config_json`, '["valid","converted"]', '["valid","won"]'),
    ...
    `published_config_json`=REPLACE(REPLACE(`published_config_json`, '["valid","converted"]', '["valid","won"]'),
                                    '"key":"converted","label":"已进入转化","sort":20,"enabled":true,"conditions":[{"field":"status","values":["converted"]}]',
                                    '"key":"won","label":"已成交","sort":20,"enabled":true,"conditions":[{"field":"status","values":["won"]}]'),
    `updater`='migration-V037', `update_time`=NOW()
...
VALUES ('core','V037','Unify customer lead opportunity and first-purchase lifecycle',
