-- V001: install the PMS (Project / Knowledge Management) module schema.
--
-- Module: pms. Depends on: core (system_users exists as an identifier target only).
-- Data scope: creates 33 PMS tables. Inserts no business rows.
-- Repeatability: every statement uses CREATE TABLE IF NOT EXISTS; re-running is a no-op.
-- Rollback limitation: dropping these tables discards all project and knowledge data;
--   take a backup first.
