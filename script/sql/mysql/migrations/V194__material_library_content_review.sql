...
-- Data scope: additive ZSJOS schema, four confirmed material type rows per enabled tenant, one
-- administrator-maintained dictionary type, the nine confirmed profession options, menu metadata,
...
-- Development-baseline correction: replay after V198 accepts only its two established creation
-- page identities sharing zsjos:material:create. Other external permission collisions still fail.
-- Apply V193 -> V194 -> V195 ... in version order; this exception does not waive prerequisites.
-- No corrective data update is needed when V194 is already complete and only its replay guard fails.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM `system_menu` WHERE `id` NOT BETWEEN 80010 AND 80040 AND `deleted`=b'0'
      ...
      -- V198 deliberately reuses the create permission for these Workbench entry pages.
      AND NOT (`permission`='zsjos:material:create' AND `type`=2 AND `parent_id`=6735
        AND COALESCE(`component`,'')='zsjos-workbench'
        AND COALESCE(`workbench_render_mode`,'')='native'
        AND ((`id`=80041 AND COALESCE(`path`,'')='viral-account-decompose'
              AND COALESCE(`component_name`,'')='ViralAccountDecomposePage')
          OR (`id`=80042 AND COALESCE(`path`,'')='viral-content-decompose'
              AND COALESCE(`component_name`,'')='ViralContentDecomposePage')))
  ) THEN
  ...
  FROM (
    SELECT '素材适配专业方向' name,'zsjos_material_profession' type
  ) seed
