-- New-media handover arbitration, review approval, and graduation business closure.
-- Dependencies: V096, V097, V102. Repeatable through information_schema guards and idempotent inserts.
-- Data scope: schema columns, one new business table, menu/button definitions, notification templates/rules.
-- Rollback is forward-only: do not drop populated columns/table; disable new permissions/rules if rollback is required.

SET @schema_name := DATABASE();

SET @sql := IF((SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=@schema_name AND table_name='zsjos_handover_sheet' AND column_name='arbitration_applicant_user_id')=0,
  'ALTER TABLE `zsjos_handover_sheet` ADD COLUMN `arbitration_applicant_user_id` bigint DEFAULT NULL, ADD COLUMN `arbitrator_user_id` bigint DEFAULT NULL, ADD COLUMN `arbitration_reason` varchar(1000) DEFAULT NULL, ADD COLUMN `arbitration_decision` varchar(32) DEFAULT NULL, ADD COLUMN `arbitration_at` datetime DEFAULT NULL, ADD KEY `idx_tenant_arbitrator_status` (`tenant_id`,`arbitrator_user_id`,`status`)', 'SELECT 1');
...
SELECT x.name,x.code,'中世健消息中心',x.scene,'in_app',x.title,x.summary,x.content,2,'["bizNo","reason"]',0,
       'V106 新媒体交接/复盘/结业消息','migration-V106',NOW(),'migration-V106',NOW(),b'0'
FROM (
 SELECT '交接待仲裁' name,'ZSJOS_MEDIA_HANDOVER_ARBITRATION_PENDING' code,'media.handover.arbitration_pending' scene,'交接待仲裁' title,'你有交接仲裁待处理' summary,'交接单 {{bizNo}} 等待你裁决。' content
 UNION ALL SELECT '交接仲裁结果','ZSJOS_MEDIA_HANDOVER_ARBITRATION_RESOLVED','media.handover.arbitration_resolved','交接仲裁结果','交接仲裁已有结果','交接单 {{bizNo}} 的仲裁已完成。'
 UNION ALL SELECT '复盘待审核','ZSJOS_MEDIA_REVIEW_PENDING','media.review.pending','复盘待审核','你有复盘报告待审核','复盘 {{bizNo}} 等待你审核。'
...
JOIN (
 SELECT '交接待仲裁' name,'media.handover.arbitration_pending' scene,'ZSJOS_MEDIA_HANDOVER_ARBITRATION_PENDING' code
 UNION ALL SELECT '交接仲裁结果','media.handover.arbitration_resolved','ZSJOS_MEDIA_HANDOVER_ARBITRATION_RESOLVED'
 UNION ALL SELECT '复盘待审核','media.review.pending','ZSJOS_MEDIA_REVIEW_PENDING'
...
SELECT 7023,'申请交接仲裁','zsjos:handover:request-arbitration',3,4,6983,'','','',NULL,0,b'1',b'1',b'1','migration-V106',NOW(),'migration-V106',NOW(),b'0'
WHERE NOT EXISTS (SELECT 1 FROM system_menu WHERE permission='zsjos:handover:request-arbitration' AND deleted=b'0');
INSERT INTO system_menu (`id`,`name`,`permission`,`type`,`sort`,`parent_id`,`path`,`icon`,`component`,`component_name`,`status`,`visible`,`keep_alive`,`always_show`,`creator`,`create_time`,`updater`,`update_time`,`deleted`)
SELECT 7024,'处理交接仲裁','zsjos:handover:arbitrate',3,5,6983,'','','',NULL,0,b'1',b'1',b'1','migration-V106',NOW(),'migration-V106',NOW(),b'0'
WHERE NOT EXISTS (SELECT 1 FROM system_menu WHERE permission='zsjos:handover:arbitrate' AND deleted=b'0');
INSERT INTO system_menu (`id`,`name`,`permission`,`type`,`sort`,`parent_id`,`path`,`icon`,`component`,`component_name`,`status`,`visible`,`keep_alive`,`always_show`,`creator`,`create_time`,`updater`,`update_time`,`deleted`)
...
SELECT 'V106','Media handover review graduation closure','V106__media_handover_review_graduation_closure.sql',NOW()
WHERE NOT EXISTS (SELECT 1 FROM `zsjos_schema_version` WHERE `version`='V106');

INSERT INTO `zsjos_module_schema_version`
(`module_code`,`version`,`description`,`checksum`,`release_version`,`installed_at`)
VALUES ('core','V106','Media handover review graduation closure',SHA2('V106__media_handover_review_graduation_closure.sql',256),'baseline',NOW())
