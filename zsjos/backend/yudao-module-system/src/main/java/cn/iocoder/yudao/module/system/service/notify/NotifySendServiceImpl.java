...
public class NotifySendServiceImpl implements NotifySendService {
    ...
    public Long sendSingleNotify(Long userId, Integer userType, String templateCode, Map<String, Object> templateParams) {
        ...
        // 发送站内信
        String title = notifyTemplateService.formatNotifyTemplateContent(
                template.getTitle() != null ? template.getTitle() : template.getName(), templateParams);
        String content = notifyTemplateService.formatNotifyTemplateContent(template.getContent(), templateParams);
        String summaryTemplate = template.getSummary() != null ? template.getSummary() : content;
        String summary = notifyTemplateService.formatNotifyTemplateContent(summaryTemplate, templateParams);
        return notifyMessageService.createNotifyMessage(NotifyMessageCreateReqDTO.builder()
                .userId(userId).userType(userType).template(template).title(title).summary(summary)
                .content(content).templateParams(templateParams).build());
    }
