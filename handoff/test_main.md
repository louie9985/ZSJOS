...
## Delivery Entry — 2026-09-22 10:16:00 +0800 — test-positioning-form-alignment

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos. Current HEAD: c44ca55f (recorded repairs and JAR ignore); positioning repair commit: d3f78206.
- User goal: Ignore the service JAR, reconcile V270 repair with handoff, and continue the authorized commit/push operation.
- Decisions/result: Confirmed the existing V270-to-V273 repair and baseline/core synchronization were complete in the worktree but previously uncommitted. Committed those exact recorded changes plus the existing ProductionTicket compile fix, migration documentation and earlier handoff records. Added *.jar before the existing Maven/Gradle wrapper exceptions; removed yudao-server.jar from the Git index only. Its runtime symlink and target remain present. No history rewrite or new branch.
- Changed files: .gitignore; yudao-server.jar (untracked from Git, local symlink retained); backend/yudao-module-zsjos/src/main/java/cn/iocoder/yudao/module/zsjos/service/production/ProductionTicketService.java; docs/operations/database-migrations.md; script/sql/mysql/migrations/V270__academic_planner_lead_ownership.sql renamed to V273__academic_planner_lead_ownership.sql; script/sql/mysql/00-bootstrap-schema.sql; script/sql/mysql/schema/core.sql; handoff/test_main.md.
- Verification evidence: zsjos_db.py check passed; old/new academic migration comparison proves only V270/V273 and temporary-name substitutions; baseline and core bytes equal; git diff --cached --check passed; staged paths explicitly checked; ignore matches JARs and preserves wrapper exceptions; local runtime symlink and target confirmed. Previous frontend 27-test/typecheck/browser evidence and unchanged ProductionTicket compile evidence reused. SQL execution/fresh-upgrade replay was not rerun or claimed; this turn commits the existing source corrections only.
- Push result: `git push origin main` failed before publication because HTTPS has no available username/credential. Default SSH has no accepted identity; the repository's configured deployment key authenticates to Gitee but Gitee explicitly reports a pull/fetch-only DeployKey. No remote URL/configuration or credentials changed, and no force push attempted. Last verified remote main remains f2e567d27ce0c37c4b16af68fda69a5e252f9ee9.
- Dependency/integration impact: No deployment, service changes, database writes, dependency additions or account/permission changes. Unrelated frontend/workbench/tsconfig.tsbuildinfo remains modified and uncommitted.
- Remaining work: Configure an existing authorized Gitee credential/key with write access, then push current main normally. The new ignore rule does not remove JAR references from historical commits.

## Workstream Registration — 2026-09-22 15:16:18 +0800 — test-legacy-order-termination-reason

- Workstream ID: `test-legacy-order-termination-reason`; Owner: Codex `/root`; Environment: test.
- Goal: Apply the user's explicit database-write request to set the termination reason of all current tenant 1 nondeleted terminated orders to “该订单为旧系统数据，已被终止”.
- Non-goals: No status, historical cancellation reason, approval record, termination time, permission, application code, migration baseline, deployment or service changes.
- Branch: main; Worktree: /opt/zsjos; Base commit: bb451ab608e7d5524e10cdec0f8a0c7d9616753e; Target branch / integration order: None.
- Ownership scope: This handoff file (append only); authorized `zsjos.zsjos_order.termination_reason` data correction; private operational SQL and original-value backup under /opt/zsjos-runtime/backups/.
- Dependencies: Existing MySQL container/configuration and UTF-8 client; no new dependencies. The user's current request explicitly authorizes this previously inspected test database target and fixed replacement text.
- Verification plan: Recheck and lock target rows; persist original values before writing; update only the frozen IDs with tenant/status/deletion guards, preserve update_time; verify exact UTF-8 text and HEX, count, idempotency and hashes of other columns before committing; verify again after commit.

## Delivery Entry — 2026-09-22 15:18:38 +0800 — test-legacy-order-termination-reason

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: bb451ab608e7d5524e10cdec0f8a0c7d9616753e (unchanged).
- User goal: Directly set all inspected terminated orders' termination reason to “该订单为旧系统数据，已被终止”.
- Key decisions: User explicitly authorized this shared test database write. Froze the 150 tenant 1 nondeleted terminated IDs after locking and confirming all original reasons were NULL. The replacement is the user's historical-data explanation, not a reconstructed historical approval reason. Preserved legacy cancel_reason, approval records, status, terminated_at, update_time and every other column.
- Execution result: Transaction committed; 150 rows updated and 150 exact replacement values verified after commit. Repeating the update inside the transaction changed zero rows.
- Changed files: handoff/test_main.md (registration and delivery only). Operational apply.sql, rollback.sql, original-values.json and verification.json are private files under /opt/zsjos-runtime/backups/order-termination-reason-20260922-151809; no production seed or migration baseline changed.
- Verification evidence: UTF-8 client plus SET NAMES utf8mb4; exact HEX comparison on all 150 values and representative Chinese-text readback passed. Before/after per-row SHA-256 comparison across all other zsjos_order columns passed; all nontarget termination reasons unchanged. All 150 terminated_at values remain NULL. Original values were persisted before the UPDATE. git diff --check passed for this record.
- Recovery: Private rollback.sql conditionally restores the original NULL reasons for the frozen tenant/status/ID scope while the value still exactly matches the replacement; review current state and obtain applicable write authorization before recovery.
- Dependency or integration impact: None; no application changes, builds, deployment, restart, dependency, Git commit or push. Existing frontend/workbench/tsconfig.tsbuildinfo modification preserved.
- Remaining work: None for this authorized data correction.

## Workstream Registration — 2026-09-22 15:23:30 +0800 — test-lead-qualification-repair

- Workstream ID: test-lead-qualification-repair; Owner: Codex /root; Environment: test.
- Goal: Execute the user's explicit database repair for tenant 1 Lead LD202609200031: restore qualification round, start/deadline, rule snapshot and matching task/event.
- Non-goals: No application, baseline, permission, ownership, follow-up, service or deployment changes. Other affected Leads remain read-only pending the requested scope decision.
- Branch: main; Worktree: /opt/zsjos; Base commit: bb451ab608e7d5524e10cdec0f8a0c7d9616753e; Target branch / integration order: None.
- Ownership scope: handoff/test_main.md (append only); the authorized Lead's four qualification columns and matching zsjos_business_task / zsjos_business_event insertions; private operational SQL, backup and verification under /opt/zsjos-runtime/backups/.
- Dependencies: Existing MySQL and active tenant default rule; no new dependency. Current rule version 1 permits 4320 minutes; original ownership yields a future deadline of 2026-09-23 14:23:22 +0800.
- Verification plan: Back up original repair fields and retain other-column hashes; replay exact SQL against session-local copies twice; lock/recheck target and rule before atomic write; assert row counts, idempotency, cross-table timestamps/round/assignee, unchanged follow-up and other columns, UTF-8 HEX, and read back after commit.

## Delivery Entry — 2026-09-22 15:27:02 +0800 — test-lead-qualification-repair

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: bb451ab608e7d5524e10cdec0f8a0c7d9616753e (unchanged).
- User goal: Direct database repair for followed-up Leads missing qualification lifecycle data, with tenant 1 / LD202609200031 explicitly identified.
- Key decisions: Applied the existing ownership-based 4320-minute calculation using the active tenant default rule (version 1), consistent with V196 backfill semantics. This is a repair using the current rule, not a claim to reconstruct an unavailable historical rule snapshot. Round 1 starts 2026-09-20 14:23:22 and ends 2026-09-23 14:23:22 +0800; deadline was future at commit. Existing first-follow-up 2026-09-20 16:05:55 and all other Lead columns are unchanged.
- Execution result: One atomic transaction updated exactly one Lead's four qualification fields and inserted exactly one pending lead_qualification task and one lead_qualification_started event with established tenant idempotency keys. Event occurrence uses original ownership; repair audit timestamps remain current and reason explicitly identifies data repair. No migration/version ledger changed.
- Changed files: handoff/test_main.md only (registration/delivery append). Private operational repair.py, apply.sql, original-values.json, replay-result.txt, apply-result.txt, verification.json and recovery.md under /opt/zsjos-runtime/backups/lead-qualification-20260922/; directory/files restricted to owner.
- Verification evidence: Exact operational SQL replayed against isolated session-local copies of the real tables; first run changed 1/1/1 Lead/task/event, second run 0/0/0. Initial replay revealed a generated-column copy restriction; the copy now uses writable columns while preserving timestamp defaults, and both replay runs passed. Real execution locked the Lead/rule and enforced fail-closed assertions; post-commit reads verified rule/round/start/deadline, unique pending task, task owner/payload, matching event, unchanged hash of every other Lead column, and UTF-8 HEX for Chinese task title/event reason. State prerequisites for qualification pass; a live user qualification command was not executed because it would make an actual valid/invalid business decision.
- Dependency/integration impact: None; no new dependency, code change, role grants, service operation, deployment, Git commit or push. Existing frontend/workbench/tsconfig.tsbuildinfo and earlier handoff edits preserved.
- Remaining work: Read-only audit found 44 other tenant 1 submitted/owned Leads with recorded first-follow-up and missing qualification deadline: 38 unexpired and 6 expired at inspection. These remain unchanged pending the asynchronously requested scope decision; filling the expired six under the existing rule would enable automatic suspension. Scoped recovery requires current-state/downstream review using the retained backup.

## Registration update — 2026-09-22 15:30:12 +0800 — test-lead-qualification-repair

- Existing owner/environment/branch/worktree/base/dependencies remain unchanged.
- User authorization: Repair all 44 remaining tenant 1 Leads identified in the preceding read-only audit; explicitly accept automatic suspension of the six already overdue under historical ownership timing.
- Scope: Freeze the 44 currently matching business numbers, repair the same four qualification fields, and insert corresponding pending qualification tasks/start events. Existing LD202609200031 is excluded. Private batch artifacts belong under /opt/zsjos-runtime/backups/lead-qualification-batch-20260922/.
- Current-state recheck: Exactly 44 pristine missing rounds, each with existing follow-up records and no qualification task/start event; current rule remains version 1 / 4320 minutes, six deadlines are in the past.
- Verification plan: Persist original fields and per-row other-column hashes; replay exact scoped SQL twice in session-local copies; lock and revalidate all frozen rows/rule; assert complete 44/44/44 updates/inserts and unchanged other fields before commit; read back UTF-8 HEX and cross-table consistency; observe existing scheduler's overdue transitions without manually deciding validity or changing deadlines/services.

## Delivery Entry — 2026-09-22 15:32:16 +0800 — test-lead-qualification-repair

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: bb451ab608e7d5524e10cdec0f8a0c7d9616753e (unchanged).
- User goal: Complete all 44 remaining missing qualification rounds, accepting automatic suspension of the six overdue Leads.
- Key decisions: Froze exactly the 44 previously identified tenant 1 business numbers; excluded the already repaired LD202609200031. Preserved original ownership and first-follow-up times; used the unchanged active default rule version 1 / 4320 minutes. Retained historical deadlines, including the six expired deadlines, as explicitly authorized.
- Execution result: One guarded transaction updated 44 Leads' four qualification fields and inserted 44 pending lead_qualification tasks plus 44 lead_qualification_started events. The existing scheduler subsequently suspended all six overdue Leads, cancelled their qualification tasks with the expected timeout reason, and recorded six corresponding round-1 suspension events. The other 38 remain submitted/owned with pending qualification tasks. Remaining tenant 1 submitted/owned Leads with a first-follow-up and missing deadline: zero.
- Changed files: handoff/test_main.md (registration update and delivery append only). Owner-restricted operational prepare.py, apply.sql, original-values.json, metadata.json, replay-result.txt, apply-result.txt, verify.py, verification.json and recovery.md are under /opt/zsjos-runtime/backups/lead-qualification-batch-20260922/.
- Verification evidence: Exact operational SQL executed twice on session-local copies: 44/44/44 first-run changes, 0/0/0 second-run changes. Real transaction locked and revalidated all 44 original row hashes and the current rule, then asserted cross-table invariants and unchanged nonqualification columns before commit. Post-commit verification passed for all 44 round/start/deadline/snapshots, unique task/event pairs, assignees, payloads, unchanged first-follow-up times and UTF-8 HEX for Chinese titles/reasons. Observed all six actual scheduler transitions and cancellation/suspension events; all other columns of the 38 unexpired Leads still match original hashes. No valid/invalid business judgment was executed.
- Dependency/integration impact: None; no source behavior, baseline/schema/version records, permissions, dependencies, services, deployment, branch, commit or push changed. Earlier handoff content and frontend/workbench/tsconfig.tsbuildinfo modification preserved.
- Remaining work: None for the authorized 44-Lead repair. Recovery is not automatic after scheduler downstream actions; any reversal needs current-state review, and business restart of a suspended Lead should use the existing supervisor restoration workflow.

## Workstream Registration — 2026-09-22 — test-content-review-errors

- Workstream ID: test-content-review-errors; Owner: Codex /root; Environment: test.
- Goal: Repair content approval error classification, field context, validation and recovery across draft/submission/review/revision/collection/publication and both BPM consumers.
- Non-goals: No Maven execution, deployment/build publication, service operations, database writes, new dependencies, permission grants, process-definition or historical-snapshot changes.
- Branch: main; Worktree: /opt/zsjos; Base commit: bb451ab608e7d5524e10cdec0f8a0c7d9616753e; Target branch/integration order: None.
- Ownership scope: content/contentreview services/controllers/errors and focused tests in backend/yudao-module-zsjos; direct content-upload validation; frontend/workbench content approval pages/components/services, API error transport and BPM error display with focused tests/browser fixtures; frontend/admin BPM error compatibility tests if needed; docs/api/content-review-batch.md; handoff/test_main.md append only.
- Dependencies: Existing Java/Spring, React/Ant Design/Vitest and available browser tooling; no additions. Preserve pre-existing tsconfig.tsbuildinfo and handoff edits.
- Verification plan: Scoped diff and source checks; existing frontend tests/typecheck; isolated browser fixture on loopback only if available; Java regression tests added/reviewed but not executed with Maven per explicit user instruction. No commands that rebuild or replace active runtime artifacts; document backend/live integration checks as unverified.


## Delivery Entry — 2026-09-22 16:48:59 +0800 — test-content-review-errors

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: bb451ab608e7d5524e10cdec0f8a0c7d9616753e (unchanged).
- User goal: Complete the authorized repair of vague content approval errors and related validation/recovery, without Maven, deployment or any impact to the running instance.
- Key decisions: Preserve the standard code/msg envelope and add optional data.fieldPath/workIndex through narrowly typed exception handlers, including errors propagated through BPM. Distinguish validation, concurrency, task ownership, configuration and publication errors. A save acknowledgment and an approval acknowledgment are separate outcomes; resolve uncertain submit responses by querying the original batch, preserve form input and block draft writes while unresolved. Keep historical dictionary snapshots, configured permission boundaries and existing workflow rules. Explicit user no-Maven/no-deployment constraint takes precedence over module build-command examples.
- Execution result: Added 39 unique actionable error codes; HTTPS no longer reports a content-version stage error, and invalid planned time no longer reports an account/count error. Added work/field context and Chinese request validation, collection guidance, task lookup distinctions, safe diagnostics and identical publication replay. Both draft editors now retain optimistic versions, prevent double save, preserve input and report preparation/save/refresh/submit failures accurately. Dictionary retries invalidate cache, preserve historical choices and display load failures. Missing tasks, history and comment load failures receive actionable feedback. Submit/query/create entry points consume existing server permissions. Vue response-interceptor compatibility was verified without adding an administration submission page.
- Changed files: Backend content-review request VOs; ContentReviewConstants/ZsjosErrorCodeConstants; ContentService/ContentVersionService; ContentReviewBatchService/ContentReviewConfigService/ContentReviewMaterialService; BusinessFileDirectUploadService; new ContentReviewFieldException and field/parameter Controller advice; focused ContentVersionServiceTest, ContentReviewBatchServiceTest, ContentReviewConfigServiceTest, BusinessFileDirectUploadServiceTest and new ContentReviewErrorContractTest. Workbench ContentApprovalDraft, StudentContentDraftPicker, BpmProcessPanel, ContentReviewBatchPage, MediaStudentsPage, services/api.ts and new contentReviewErrors.ts/tests; isolated test/content-review-errors.tsx, content-review-errors-browser.mjs and content-review-admin-errors.mjs. docs/api/content-review-batch.md and this append-only handoff record. Pre-existing tsconfig.tsbuildinfo and earlier handoff entries preserved.
- Verification evidence: Isolated javac compiled 17 changed main sources and six focused test classes using existing JDK/cached dependencies/project classes into /tmp/zsjos-content-review-check, with no Maven or target output writes. JUnit Launcher: 72 tests passed, including error contracts/privacy, no mutation on invalid input, version/ownership rejection, BPM task distinctions, collection/configuration/upload behavior and published-result replay. Workbench tsc --noEmit --incremental false passed; five focused Vitest files / 39 tests passed with --configLoader runner. Actual Chromium fixture at 1440 and 390 widths passed HTTPS/time validation, work-field highlighting, input retention, duplicate-click locking, saved-but-rejected retry/version handling, failed post-save read, changed state, historical dictionaries, lost-response reconciliation, uncertain submit and dictionary errors; screenshots visually inspected with complete Chinese glyphs. Vue actual interceptor isolated execution passed for three new error codes, optional details, preserveBusinessError and success envelopes. UTF-8 source decoding, 39-code uniqueness and git diff --check passed. Temporary evidence lives in /tmp/zsjos-content-review-check and /tmp/zsjos-content-error-browser; screenshots /tmp/zsjos-content-errors-desktop-field.png and /tmp/zsjos-content-errors-mobile-uncertain.png.
- Verification findings: First isolated Java run found obsolete expected error codes and a mock review item missing sortNo; corrected fixtures and added permission/task/publication regressions, final 72 passed. Browser checks exposed cached dictionary reload and a client 403 message override, both corrected and rerun successfully. The pre-existing large test font was truncated; a complete test-only subset in /tmp enabled visual inspection. An earlier broad frontend test attempt encountered untouched typography guard and finance table value failures; they were not changed and are not included in focused pass counts.
- Dependency/integration impact: No dependency additions, SQL/database writes, permission grants, service starts/stops/reconfiguration, production build, deployment, Git branch/worktree operation, commit or push. Only isolated ephemeral test processes and loopback fixture server were used and closed. Both React and Vue consumers verified at the error-transport boundary.
- Remaining work: None for authorized source repair and isolated verification. Running application is deliberately unchanged. Full dependency-graph/package verification, real Spring MVC/BPM/database end-to-end commands and post-deployment acceptance remain unverified under the no-Maven/no-runtime-impact constraint; cached-class javac/unit tests and synthetic browser transport do not substitute for those checks. Any later deployment is a separate operation.


## Workstream Registration — 2026-09-23 02:19:14 +0800 — test-migration-schema-numbering

- Workstream ID: test-migration-schema-numbering; Owner: Codex /root; Environment: test (resolved from /etc/zsjos/agent-environment).
- Goal: Repair the confirmed V274 desired-schema synchronization omission; audit all module migration numbering, references and registrations, and repair established source inconsistencies.
- Non-goals: No shared database writes, checksum reconciliation, applied migration rewrite, business behavior changes, role grants, deployment, shared service operation, dependency additions, Git branch/commit/push operations.
- Branch: main; Worktree: /opt/zsjos; Base commit: 0af11251937b06b469adb4a3377715736484f24f; Target branch/integration order: None.
- Ownership scope: script/sql/mysql/schema/core.sql; script/sql/mysql/tools/zsjos_db.py and focused migration CLI regression tests; directly affected database migration documentation; handoff/test_main.md append only. Further SQL changes require an evidence-based scope update before edits.
- Dependencies: Existing Python standard library, Docker and locally available MySQL image; preserve pre-existing frontend/workbench/tsconfig.tsbuildinfo modification.
- Verification plan: All-module sequence/reference audit, static check, focused CLI regression tests, isolated fresh/upgrade/guardrail database checks and scoped read-only comparison with the configured test database; inspect SQL results and UTF-8 HEX. Record existing chain blockers separately from verified fixes. Temporary fixture databases/containers contain only generated test data; shared databases remain read-only.

## Registration update — 2026-09-23 02:21:19 +0800 — test-migration-schema-numbering

- Scope adds script/sql/mysql/00-bootstrap-schema.sql: isolated fresh/upgrade/guardrail execution found six missing CREATE TABLE terminators in the V214/V264 delivery-table definitions shared by baseline and desired schema. Correct only these syntax omissions in both copies; preserve numbered migration bytes. Verification expands to the full applicable fresh chain and scoped schema comparison; shared database remains read-only.

## Delivery Entry — 2026-09-23 02:28:29 +0800 — test-migration-schema-numbering

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: 0af11251937b06b469adb4a3377715736484f24f (unchanged).
- User goal: Repair the Core desired-schema/baseline mismatch and inspect/fix other migration-number inconsistencies.
- Key decisions: Synchronize the existing V274 reply definition into desired schema instead of generating a reverse migration. Restore missing SQL terminators in six V214/V264 delivery-table baseline definitions. Remove the generator's obsolete odd-version reservation in favor of the documented consecutive-version rule. Preserve all numbered migration bytes and deployed checksum records; no new migration or renumbering was necessary.
- Execution result: Desired schema and fresh baseline are byte-identical. All 294 migration filenames are continuous and unique: Core V001–V277, EAM V001–V012, HRM/FMS V001–V002 and PMS V001. Referenced SOURCE SQL files exist; the scoped version-condition audit found no later-version equality prerequisites. Missing literal self-registration in executor-owned files is not treated as a missing version. Read-only configured-test ledger audit found exactly Core V274–V277 pending and no non-seed file checksum mismatches.
- Changed files: script/sql/mysql/schema/core.sql; script/sql/mysql/00-bootstrap-schema.sql; script/sql/mysql/tools/zsjos_db.py; new script/sql/mysql/tools/test_migration_numbering.py; docs/operations/database-migrations.md; handoff/test_main.md (append only). Pre-existing frontend/workbench/tsconfig.tsbuildinfo preserved.
- Verification evidence: CLI check PASS; numbering regression 4 tests PASS (both next-version parities plus duplicate/gap/start rejection); authorization SQL policy 4 tests PASS; test-guardrails PASS; scoped git diff --check and baseline byte comparison PASS. Isolated MySQL executed the corrected bootstrap, recreated V273 assistance shape in an empty fixture and executed V274–V277 twice with runner ledger recording. Scoped final columns/indexes equal baseline, notification/template rows and menu identities stable on replay, Chinese menu HEX correct, role-grant count unchanged, and all four module checksums equal file bytes. Calendar metadata prerequisite was loaded from the exact V146 menu INSERT into the isolated fixture. Shared-test read-only comparison found zero column/index differences for all six delivery tables; assistance differs only by the pending V274 seven columns and four index members. Shared Chinese table comments inspected via HEX.
- Verification limits/root causes: Full test-fresh remains blocked by existing V150 preflight rejecting menu 79920 with V185's manage-all permission. Legacy test-upgrade exercises only V019–V021 but then invokes current global verification without later migrations, producing later-version assertion failures. Neither is reported as PASS. First isolated runs exposed the six missing statement terminators, now corrected and rerun. Initial scoped fixture lacked the historical V146 calendar directory; supplied its exact menu SQL and reran successfully. Source-reference audit was narrowed to SOURCE filename.sql statements after initially matching a column named source. Database authentication was resolved through the existing container password-file configuration without logging its value.
- Evidence artifacts: /tmp/zsjos-migration-fresh.log; /tmp/zsjos-migration-upgrade.log; /tmp/zsjos-migration-guardrails.log; /tmp/zsjos-migration-scoped.py; /tmp/zsjos-migration-scoped.log; /tmp/zsjos-migration-schema-comparison.json. Isolated test containers were removed by the test harness; no shared service was altered.
- Dependency/integration impact: No new dependency, shared database mutation, role assignment, applied migration rewrite, checksum reconciliation, deployment, branch/worktree operation, commit or push. Deployment must rebuild its migrator artifact from the corrected source through the separately authorized release process.
- Remaining work: Historical fresh-chain V150/V185 compatibility and legacy upgrade-verifier scope remain unresolved; complete release fresh/upgrade acceptance is not established. Shared test remains at V273; applying V274–V277 (including notification/dictionary configuration and scoped Lead-stage initialization) requires separately scoped shared-state authorization and preflight review. The original static error and identified source/number-generator corrections are verified locally.

## Registration update — 2026-09-23 02:35:21 +0800 — test-migration-schema-numbering

- User goal extends to repairing the reproduced fresh menu-ID collision and the legacy upgrade test's verification scope.
- Scope adds script/sql/mysql/01-bootstrap-system-seed.sql and focused migration execution tests/fixtures; existing CLI, baseline, documentation and handoff scope retained. Owner, environment, branch/worktree and HEAD unchanged.
- Correction to prior delivery: V150 was not blocked by V185 manage-all metadata. Stepwise isolated reproduction shows bootstrap assigns AUTO_INCREMENT 79920 to V276's zsjos:sales-performance-target:configure (parent 79914); V150 then rejects that identity. Evidence: /tmp/zsjos-v150-diagnose.log. The previous V150/V185 root-cause claim is withdrawn; history remains append-only.
- Plan: Defer duplicate V276 bootstrap menu creation to its owning numbered migration; verify older fixed menu identities stay available. Replace unrelated latest-version checks in V019–V021 upgrade test with assertions for that upgrade's schema, version records, data uniqueness and replay. Correct active documentation. Run controlled fresh, upgrade, guardrail and focused tests, and compare affected menu definitions read-only with shared test. No shared writes, historical migration rewrite or checksum reconciliation is authorized.

## Registration update — 2026-09-23 02:37:31 +0800 — test-migration-schema-numbering

- Scope adds script/sql/mysql/verify-bootstrap.sql: removing premature V276 menu seeds lets all 293 pending migrations execute; final verifier has one stale V080 template assertion after V257 intentionally adds submitterIdentityLabel. Synchronize that assertion with the established V257 contract, retaining strict old/new version-specific checks. No notification data or numbered migration changes.

## Delivery Entry — 2026-09-23 02:42:40 +0800 — test-migration-schema-numbering

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; existing registration retained. HEAD: 0af11251937b06b469adb4a3377715736484f24f (unchanged).
- User goal: Repair the concretely diagnosed fresh menu-ID collision and legacy upgrade-test verification mismatch.
- Key decisions: V276 alone creates its eight menu/button definitions after earlier migrations establish fixed IDs. Remove only duplicate premature inserts from fresh System seed. Retain V150/V185 historical identities, every numbered migration's bytes and deployed checksums. Scope the V019-to-V021 test to its four affected tables, ledger entries, uniqueness, tenant separation and replay. Synchronize V080 provider-template verification to the V257 identity wording when V257 is installed.
- Correction of prior report: Actual bootstrap collision was 79920 = zsjos:sales-performance-target:configure (V276), conflicting with V150's zsjos:partner:manage, not V185's manage-all state. Updated active operations documentation; prior append-only history is corrected by this entry and the registration update above.
- Execution result: Fresh bootstrap plus all 293 pending migrations completes through Core V277 and the manifest module chains. Final menu 79920 retains zsjos:partner:manage-all under Partner page 6852; all eight V276 permissions exist exactly once and the configuration label passes UTF-8 HEX. V019-to-V021 now validates its actual contract without asserting unexecuted later migrations. Default notification verifier still rejects old wording after V257, wrong identity parameters and wrong summaries.
- Changed files this turn: script/sql/mysql/01-bootstrap-system-seed.sql; script/sql/mysql/tools/zsjos_db.py (fresh menu regression and scoped upgrade assertions); script/sql/mysql/verify-bootstrap.sql; docs/operations/database-migrations.md; handoff/test_main.md append only. All earlier source edits and the pre-existing frontend/workbench/tsconfig.tsbuildinfo modification preserved.
- Verification evidence: Final test-fresh PASS (all 293 pending SQL files, final Core verification, explicit Partner/V276 identity and UTF-8 assertions, non-empty bootstrap rejection). test-upgrade PASS (full columns/indexes comparison for four tables, both version ledgers, logical-delete reuse, duplicate active product rejected with MySQL 1062, same key allowed in another tenant, V020/V021 replay preserving schema/rows). test-guardrails PASS. CLI check PASS; numbering 4 tests and authorization SQL policy 4 tests PASS; git diff --check PASS. Isolated template-verifier regression PASS for both version contracts and three negative cases, with UTF-8 HEX. A first full fresh attempt after menu repair exposed only the stale V080 assertion; updated it from the V257 migration contract and reran successfully, then reran fresh after adding explicit menu regression checks.
- Scoped shared-test comparison: Read-only query confirms historical 79920 manage-all and 79997 manage remain type-3 children of 6852 with correct Chinese HEX. Shared Core remains V273 and has zero V276 permission entries, consistent with the unapplied V276 migration; no existing menu or grant was corrected or reassigned. Schema checks from preceding delivery remain valid for unchanged table definitions.
- Evidence artifacts: /tmp/zsjos-fresh-menu-fix.log; /tmp/zsjos-upgrade-scope-fix.log; /tmp/zsjos-menu-fix-guardrails.log; /tmp/zsjos-template-verifier-test.py and .log; initial diagnosis /tmp/zsjos-v150-diagnose.log. Tests used isolated generated databases and their containers were removed.
- Dependency/integration impact: None. No new dependency, shared database write, role grant, historical migration rewrite, checksum reconciliation, build/publication, service operation, branch/worktree operation, commit or push. Backend/frontend API, authentication and permission semantics are unchanged; the correction concerns fresh SQL ordering and verification only.
- Remaining work: None for these source fixes and applicable isolated checks. Running shared test remains unchanged at V273; applying V274–V277 or deploying a newly built migrator remains a separately authorized shared-state operation. Passing these database checks does not claim application release/browser acceptance.


## Workstream Registration — 2026-09-23 02:52:53 +0800 — test-lead-first-follow-recovery

- Workstream ID: test-lead-first-follow-recovery; Owner: Codex /root; Environment: test.
- Goal: Execute the user-authorized tenant 1 five-table first-follow-up and missing qualification recovery using the supplied unchanged SQL. Restart wholly missing qualification rounds from the actual repair time and preserve existing rounds.
- Non-goals: No business code, schema/migration rollout, ownership reconstruction, validity decisions, role grants, service/deployment, Git branch/commit/push or unrelated worktree edits.
- Branch: main; Worktree: /opt/zsjos; Base/HEAD: 0af11251937b06b469adb4a3377715736484f24f; Target branch/integration order: None.
- Ownership scope: handoff/test_main.md append only; authorized tenant 1 repair writes to zsjos_lead, zsjos_lead_follow_up_record, zsjos_business_task, zsjos_business_event and zsjos_performance_attribution; restricted operational backup/evidence under /opt/zsjos-runtime/backups/lead-first-follow-recovery-20260923/. Preserve all pre-existing source, handoff and build-info edits.
- Dependencies: Existing UTF-8 MySQL client/container and unchanged script/sql/mysql/repairs/lead_first_follow_recovery.sql (SHA-256 b7e0edb2ae4f2e1d0a5689021c0c15e46a34dd788183f60410329c1e0315c696). Read-only inspection now confirms V274–V277 recorded and all nine prerequisites present/InnoDB. Rule version 1 uses 30/4320 minutes; no performance organization mappings exist, so no center identity will be inferred.
- Verification plan: Private tenant-scoped five-table dump; preflight conflict and candidate audit; transaction preview plus full rollback; fresh-connection official execution with new Beijing repair time and fail-closed same-session before/after checks; preserve unrelated fields and existing rounds; verify task/event/attribution relationships and Chinese HEX; post-commit exact readback and full-script repeat with rollback and 0/0/0 changes.


## Delivery Entry — 2026-09-23 02:59:31 +0800 — test-lead-first-follow-recovery

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: 0af11251937b06b469adb4a3377715736484f24f (unchanged).
- User goal: Finish the already authorized five-table database recovery after prerequisite deployment, without changing application code.
- Key decisions: Read-only recheck found V274–V277 already installed and all nine prerequisites InnoDB. Executed the supplied unchanged repair SQL for tenant 1 only. Kept existing qualification rounds and ownership_started_at; no Lead validity decision. New rounds use actual Beijing repair time. Zero configured performance organization mappings means center snapshots remain NULL; current System user/department snapshots are used without inventing mappings.
- Execution result: Preview passed and rolled back; official transaction committed with repair time 2026-09-23 02:57:02 +08:00. Recovered 191 first-follow facts and matching record markers; inserted 191 completed first-follow tasks, 69 pending first-follow tasks, 257 qualification tasks, 257 start events and 257 QUALIFICATION snapshots. Updated 260 distinct Leads; no existing business tasks/events/attributions were modified. New qualification deadline 2026-09-26 02:57:02; new pending first-follow deadline 2026-09-23 03:27:02. All 126 pre-existing complete qualification rounds across tenant 1 retain their four qualification fields.
- Specific acceptance: LD202609170026 first-follow restored to 2026-09-18 17:57:49, ownership start still NULL, new round uses the official repair time. LD202609200031 remains valid with original deadline 2026-09-23 14:23:22. Remaining submitted/owned Leads with current-owner/current-cycle evidence but missing first-follow: zero; remaining submitted/owned Leads missing qualification deadline: zero.
- Changed files: This handoff append only. Restricted operational executor, unchanged executed SQL copy, tenant five-table dump, before/after images, verification summaries and recovery instructions are under /opt/zsjos-runtime/backups/lead-first-follow-recovery-20260923/. All pre-existing repository edits preserved.
- Verification evidence: Dump exit success, all five table sections and completion footer, 12,956,068 bytes with recorded SHA-256. Lead-first transaction locks and rule/user/department/org shared locks; preflight rejects partial/conflicting task/event cases. Every existing row/column of the five tenant tables compared against exact expected changes; all new task/event/attribution fields and associations verified before COMMIT. UTF-8 read plus Chinese title HEX E69C89E69588E680A7E588A4E5AE9AEFBC9A verified for all 257 new qualification tasks. Postcommit readback exactly matched transaction results. Full unchanged SQL re-executed in a fresh transaction: 0/0/0, no five-table row changes; ROLLBACK and exact readback passed.
- Verification correction: First preview rolled back because local verification compared zero-microsecond MySQL JSON datetime formatting with second-resolution values. Normalized equivalent datetimes in the private checker, confirmed no committed recovery rows/tasks, and reran successfully. Supplied SQL was unchanged throughout. Preview auto-increment gaps were not reset.
- Dependency/integration impact: None; no code, schema, numbered migration, dictionary, permission grant, service/deployment, dependency, branch/worktree, commit or push changes by this task. Existing migrations were observed, not executed here. Frontend button permissions and running backend code were not verified by this database-only operation.
- Remaining work: None for the authorized database repair. Postcommit recovery requires checking intervening business/scheduler actions and guarded per-record compensation from private before-images; never whole-table restore or deletion by repair marker.

## Workstream Registration — 2026-09-23 10:16:39 +0800 — test-workbench-lead-calendar-route

- Workstream ID: test-workbench-lead-calendar-route; Owner: Codex /root; Environment: test.
- Goal: Make the authorized Workbench sales lead follow-up calendar menu render for users whose server menu permissions already include the page.
- Non-goals: No backend/API, database, role/menu assignment, authentication, tenant, layout publication, dependency, service, branch, commit, push or unrelated frontend changes.
- Branch: main; Worktree: /opt/zsjos; Base/HEAD: 0af11251937b06b469adb4a3377715736484f24f; Target branch/integration order: None.
- Ownership scope: frontend/workbench/src/constants.ts; directly affected Workbench menu coverage test assertions; this handoff append only.
- Dependencies: Existing server menu 602327 and permission `zsjos:lead-follow-up-calendar:query`; no new dependency.
- Verification plan: focused Workbench menu tests, typecheck if practical, and diff/whitespace checks.

## Delivery Entry — 2026-09-23 10:24:32 +0800 — test-workbench-lead-calendar-route

- Owner/environment/branch/worktree: Codex /root; test; main; /opt/zsjos; registration metadata unchanged. HEAD: 0af11251937b06b469adb4a3377715736484f24f (unchanged).
- User goal: Make the authorized Workbench sales lead follow-up calendar visible to the sales specialist role.
- Key decisions: Keep server authorization and menu metadata unchanged. Add the existing `/calendar/sales-lead-follow-up` route constant to the Workbench `RENDERABLE_APP_ROUTES` allowlist so the existing server-returned menu is retained during navigation filtering. Update the focused route coverage assertion for the current 73-route set.
- Execution result: Workbench no longer removes the authorized sales lead follow-up calendar leaf during navigation construction; its existing component registry and RouteHost path remain unchanged.
- Changed files: frontend/workbench/src/constants.ts; frontend/workbench/src/services/menu.test.ts; handoff/test_main.md append only. Pre-existing unrelated source, SQL, documentation and build-info changes were preserved.
- Verification evidence: `npm test -- src/services/menu.test.ts src/services/menuComponentRegistry.test.ts --configLoader runner` passed with 27 tests; `npm run typecheck` passed; scoped `git diff --check` passed. The default Vitest config loader was initially blocked by the pre-existing root-owned `frontend/workbench/node_modules/.vite-temp`; the same tests passed with Vitest's runner config loader without modifying dependencies.
- Dependency/integration impact: None. No backend/API, database, role/menu assignment, layout publication, authentication, tenant, dependency, service, branch, commit or push operation. A rebuilt/reloaded Workbench bundle is required for the browser to receive the allowlist fix.
- Remaining work: Deploy or restart the frontend asset serving this Workbench build, then refresh the sales specialist session and verify the menu in the browser. Runtime/browser acceptance was not performed in this source-only turn.
