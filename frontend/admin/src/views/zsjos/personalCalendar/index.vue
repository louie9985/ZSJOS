<template>
  <ContentWrap>
    <div class="heading"><div><h2>我的日历</h2><span>仅显示当前登录用户的个人日程</span></div><div><el-button @click="load">刷新</el-button><el-button v-hasPermi="['zsjos:personal-calendar:create']" type="primary" @click="openCreate">新增日程</el-button></div></div>
    ...
    <el-calendar v-else v-model="anchor" v-loading="loading" class="mt-16px">
      <template #date-cell="{ data }"><div class="date-cell"><strong>{{ dayjs(data.day).date() }}</strong><div v-for="event in eventsFor(data.day)" :key="event.id" class="event"><span>{{ event.title }}</span><span><el-button v-hasPermi="['zsjos:personal-calendar:update']" link :icon="Edit" @click.stop="openEdit(event)" /><el-button v-hasPermi="['zsjos:personal-calendar:delete']" link type="danger" :icon="Delete" @click.stop="remove(event)" /></span></div></div></template>
    </el-calendar>
  </ContentWrap>
...
const load=async()=>{loading.value=true;error.value='';const value=dayjs(anchor.value);try{events.value=await CalendarApi.getPersonalCalendar({rangeStart:value.startOf('month').startOf('week').format('YYYY-MM-DDTHH:mm:ss'),rangeEnd:value.endOf('month').endOf('week').add(1,'second').format('YYYY-MM-DDTHH:mm:ss')})}catch(e:any){events.value=[];error.value=e?.msg||e?.message||'个人日程加载失败'}finally{loading.value=false}}
...
watch(anchor,load);onMounted(load)
</script>
