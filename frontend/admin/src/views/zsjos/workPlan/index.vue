<template>
  <ContentWrap>
    <el-form :model="query" inline>
      ...
      <el-form-item><el-button type="primary" @click="search">查询</el-button><el-button @click="reset">重置</el-button><el-button v-hasPermi="['zsjos:work-plan:export']" :loading="exportLoading" @click="exportTasks">导出任务明细</el-button></el-form-item>
    </el-form>
  </ContentWrap>
...
const message = useMessage(); const query = reactive<WorkPlanApi.WorkPlanPageReqVO>({ pageNo: 1, pageSize: 10 }); const plans = ref<WorkPlanApi.WorkPlanVO[]>([]); const total = ref(0); const loading = ref(false); const exportLoading = ref(false); const error = ref(''); const templates = ref<ConfigApi.WorkPlanTemplateVO[]>([]); const users = ref<UserApi.UserVO[]>([]); const departments = ref<DeptApi.DeptVO[]>([]); const detail = ref<WorkPlanApi.WorkPlanVO>(); const detailOpen = ref(false); const detailLoading = ref(false); const transferOpen = ref(false); const transferTask = ref<WorkPlanApi.WorkTaskVO>(); const transferUserId = ref<number>(); const transferReason = ref('')
...
const openTransfer = (task: WorkPlanApi.WorkTaskVO) => { transferTask.value = task; transferUserId.value = task.assigneeUserId; transferReason.value = ''; transferOpen.value = true }; const transfer = async () => { if (!transferTask.value || !transferUserId.value || !transferReason.value.trim()) return message.warning('请选择责任人并填写原因'); await WorkPlanApi.adjustWorkTask(transferTask.value, transferUserId.value, transferReason.value.trim()); message.success('工作任务已转派'); transferOpen.value = false; if (detail.value) await openDetail(detail.value); await loadPlans() }; const exportTasks = async () => { exportLoading.value = true; try { const data = await WorkPlanApi.exportWorkPlans(query); download.excel(data, '计划任务明细.xlsx'); message.success('导出成功') } finally { exportLoading.value = false } }
