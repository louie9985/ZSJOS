<template>
  <ContentWrap>
    ...
    <el-alert v-if="error" type="error" :title="error" show-icon :closable="false" class="mb-12px">
      <template #default
        ><el-button link type="primary" @click="loadList">重试</el-button></template
      >
    </el-alert>
    <el-table v-loading="loading" :data="list" empty-text="暂无异常客资">
      ...
      <el-table-column label="客户" min-width="150">
        <template #default="scope"
          ...
          ><div>{{ scope.row.submittedMobile || '无手机号' }}</div></template
        >
      </el-table-column>
      <el-table-column label="当前阶段" width="130"
        ><template #default="scope"
          ><el-tag type="warning">{{ stageLabel(scope.row.handlingStage) }}</el-tag></template
        ></el-table-column
      >
      <el-table-column
      ...
      <el-table-column label="判定截止" min-width="165"
        ><template #default="scope">{{
          formatZsjosTimestamp(scope.row.qualificationDeadlineAt)
        }}</template></el-table-column
      >
      <el-table-column label="挂起时间" min-width="165"
        ><template #default="scope">{{
          formatZsjosTimestamp(scope.row.suspendedAt)
        }}</template></el-table-column
      >
      <el-table-column label="操作" width="270" fixed="right">
        <template #default="scope">
          <el-button
            v-if="exceptionType === 'suspended'"
            link
            type="primary"
            v-hasPermi="['zsjos:lead:qualification:manage']"
            @click="openAction(scope.row, 'restore')"
            >恢复</el-button
          >
          <el-button
            link
            type="primary"
            v-hasPermi="['zsjos:lead:qualification:manage']"
            @click="openAction(scope.row, 'transfer')"
            >转派</el-button
          >
          <el-button
            v-if="exceptionType === 'suspended'"
            ...
            type="warning"
            v-hasPermi="['zsjos:lead:qualification:manage']"
            @click="openAction(scope.row, 'recycle')"
            >回收</el-button
          >
          <el-button
            link
            type="success"
            v-hasPermi="['zsjos:lead:qualification:manage']"
            @click="openAction(scope.row, 'release')"
            >释放</el-button
          >
        </template>
      </el-table-column>
      ...
      <el-form-item v-if="action === 'transfer'" label="目标销售" required>
        ...
        >
          <el-option
            ...
            :key="item.id"
            :label="`${item.nickname}${item.deptName ? ` · ${item.deptName}` : ''}`"
            :value="item.id"
          />
        </el-select>
      </el-form-item>
      <el-form-item label="处置理由" required
...
const loadList = async () => {
  ...
  error.value = ''
  try {
    const data = await LeadApi.getQualificationExceptionPage({
    ...
    error.value = e?.msg || e?.message || '异常客资加载失败'
    list.value = []
    total.value = 0
  } finally {
    loading.value = false
...
const openAction = async (row: LeadApi.LeadQualificationExceptionVO, nextAction: Action) => {
  ...
  try {
    candidates.value = (await LeadApi.getTransferCandidates(
      row.id
    )) as LeadApi.LeadTransferCandidateVO[]
  } catch (e: any) {
    message.error(e?.msg || e?.message || '目标销售加载失败')
  } finally {
...
const submitAction = async () => {
  ...
  try {
    ...
    if (action.value === 'transfer')
      await LeadApi.transferLead(selected.value.id, { ...command, salesUserId: salesUserId.value! })
...
const ownerText = (row: LeadApi.LeadQualificationExceptionVO) =>
  exceptionType.value === 'suspended'
    ? row.ownerUserName || (row.ownerUserId ? `用户 #${row.ownerUserId}` : '未分配')
    : row.recycleSourceOwnerUserName ||
      (row.recycleSourceOwnerUserId ? `用户 #${row.recycleSourceOwnerUserId}` : '-')
onMounted(loadList)
</script>
