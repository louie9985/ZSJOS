...
const message = useMessage(); const loading = ref(false); const saving = ref(false); const copying = ref(false); const publishing = ref(false); const error = ref(''); const config = ref<Api.Config>(); const draft = computed(() => config.value?.draft)
...
const persistDraft = async (value: Api.Version) => { if (!validateDraft(value)) throw new Error('请检查时间限制和首联任务清单'); await Api.saveStudentContactDraft({ ...value, idempotencyKey: crypto.randomUUID() }) }
