...
export const saveStudentContactDraft = (data: { id: number; version: number; idempotencyKey: string; firstContactTimeoutMinutes: number; studyPlanTimeoutMinutes: number; checklist: ChecklistItem[]; quickNotes: string[]; collaboratorTabs: Record<string, string[]> }) => request.put<boolean>({ url: '/zsjos/student-contact-config/draft', data })
