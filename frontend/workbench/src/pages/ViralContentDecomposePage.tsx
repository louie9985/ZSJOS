...
export default function ViralContentDecomposePage() {
  ...
  return <section className="workspace-page standalone-viral-decompose"><ViralContentMaterialForm key={formKey} mode="create" type={type} dicts={dicts} onRetry={() => void load()} onClose={() => setFormKey(value => value + 1)} onSaved={({ submitted }) => { if (submitted) message.success('已提交审批'); setFormKey(value => value + 1) }} /></section>
}
