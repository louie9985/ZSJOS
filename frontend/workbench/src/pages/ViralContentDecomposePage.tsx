...
export default function ViralContentDecomposePage() {
  ...
  if (loading) return <Spin />; if (error) return <Alert type="error" showIcon message={error} />; if (!type) return <Empty description="爆款内容模板尚未发布" />
  return <section className="workspace-page standalone-viral-decompose"><ViralContentMaterialForm key={formKey} mode="create" type={type} dicts={dicts} onClose={() => setFormKey(value => value + 1)} onSaved={() => { message.success('已保存，继续创建下一条'); setFormKey(value => value + 1) }} /></section>
}
