...
export default function SalesOrderApprovalPage({ permissions }: { permissions: string[] }) {
  ...
  const reload = () => { void loadProfile(); if (groupKey && center) void loadPage(undefined, true) }
  ...
  return <section className="workspace-page lead-management-page sales-order-approval-page">
    ...
    <div className="lead-inbox-layout"><aside className="lead-inbox-list-pane"><div className="lead-inbox-toolbar"><AdvancedFilterToolbar scene="order" placeholder="搜索订单号 / 学员姓名 / 手机号" keyword={keyword} value={advancedFilter} onKeyword={setKeyword} onChange={setAdvancedFilter}/>{center === 'finance' && permissions.includes('zsjos:export:finance-order') && <Button icon={<DownloadOutlined/>} onClick={exportFinanceOrders}>导出</Button>}</div>
      {error && <Alert className="lead-list-error" type="error" showIcon message={error} action={<Button size="small" onClick={() => void loadPage(undefined, true)}>重试</Button>}/>}
      <div className="lead-inbox-scroll" onScroll={event => { const node = event.currentTarget; if (!loading && hasMore && cursor && node.scrollHeight - node.scrollTop - node.clientHeight < 80) void loadPage(cursor, false) }}>
