...
import { ApiError, api, type DictData, type ManagedLead, type MediaAccountField, type MediaAccountFieldConfig, type MediaStudentDetail, type MediaStudentTalkRecord, type MyStudent } from '../services/api'
...
export default function MediaStudentsPage({ permissions = [] }: { permissions?: string[] }) {
  ...
  const [leadDetail, setLeadDetail] = useState<ManagedLead>(), [selectedServiceId, setSelectedServiceId] = useState<number>()
  ...
  const loadDetail = useCallback(async (personId: number, preferredServiceId?: number) => {
    ...
    try {
      ...
      if (run === detailRun.current) { setDetail(value); setTalks(records); setSelectedServiceId(service?.serviceRelationId); setLeadDetail(lead) }
    }
  ...
  const submit = async () => { if (!detail || !dialog) return
    try { const values = await form.validateFields(); setSaving(true)
      ...
      setDialog(undefined); message.success('已保存'); await loadDetail(detail.student.personId, selectedServiceId)
  ...
  const contentAction = async (row: MediaStudentDetail['contents'][number], action: string) => { try {
    ...
    if (selectedId) await loadDetail(selectedId, selectedServiceId) } catch (cause) { message.error(errorText(cause)) } }
  ...
  const selectService = async (serviceRelationId: number) => {
    ...
    setSelectedServiceId(serviceRelationId); setDetailLoading(true); setDetailError(''); setLeadError('')
    try {
      const lead = service.leadId ? await api.managedLead(service.leadId) : undefined
      if (run === detailRun.current) setLeadDetail(lead)
    } catch (cause) {
      if (run === detailRun.current) {
        setLeadDetail(undefined)
        setLeadError(cause instanceof ApiError && cause.code === 403 ? '无权查看该课程客资' : errorText(cause))
      }
    } finally { if (run === detailRun.current) setDetailLoading(false) }
  }
