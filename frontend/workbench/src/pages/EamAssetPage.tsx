...
import { api, type EamAssetItem, type EamAssetSummary, type EamCategory, type EamCategoryField, type EamDemand, type EamDemandItem, type EamStockCandidate } from '../services/api'
...
export default function EamAssetPage({ permissions, view }: { permissions: string[]; view: 'assets' | 'demands' }) {
  ...
  const load = async () => {
    ...
    try {
      if (view === 'assets') setSummary(await api.eam.myAssets())
  ...
  return <section className="workspace-page eam-assets-page">
    <div className="eam-page-toolbar"><h1>{view === 'assets' ? '我的资产' : '采购申请'}</h1><Space><Button icon={<ReloadOutlined/>} onClick={load}>刷新</Button>{view === 'demands' && permissions.includes('eam:workbench:demand:create') && <Button type="primary" icon={<PlusOutlined/>} onClick={() => setEditorOpen(true)}>新建申请</Button>}</Space></div>
