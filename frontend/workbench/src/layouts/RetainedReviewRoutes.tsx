...
const RetainedPageActive = createContext(true)
export const useRetainedPageActive = () => useContext(RetainedPageActive)

/** A frozen route location prevents hidden pages reacting to another tab's query. */
export default function RetainedReviewRoute({ active, children }: { active: boolean; children: ReactNode }) {
  ...
  return <div hidden={!active} style={active ? { height: '100%', minHeight: 0 } : { display: 'none' }}>
    <RetainedPageActive.Provider value={active}>
      <Routes location={retained.current}><Route path="*" element={children} /></Routes>
    </RetainedPageActive.Provider>
  </div>
}
