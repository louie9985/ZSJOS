...
describe('lead submitter assistance request', () => {
  it('keeps the projected action available in normal and manager detail modes', () => {
    expectSourceToContainTokens(detail, "item.code === 'REQUEST_SUBMITTER_ASSIST'")
    expectSourceToContainTokens(detail, "actions.has('REQUEST_SUBMITTER_ASSIST')")
  ...
  it('requires the two business text fields and uploads optional attachments before submit', () => {
    ...
    expectSourceToContainTokens(detail, 'if (!assistProblem.trim() || !assistExpected.trim())')
  ...
  it('keeps the API and todo deep-link protocol aligned', () => {
    ...
    expectSourceToContainTokens(todayTasks, "task.actionCode === 'OPEN_LEAD_SUBMITTER_ASSIST'")
  })
})
