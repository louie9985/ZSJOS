...
describe('buildCrmVars across default presets', () => {
  ...
  it('glass mode overrides only surfaces, leaving text and primary intact', () => {
    ...
    expect(changed.sort()).toEqual(['--crm-bg-chrome', '--crm-bg-container', '--crm-bg-elevated', '--crm-bg-layout', '--crm-bg-mask', '--crm-bg-overlay', '--crm-bg-sunken', '--crm-border', '--crm-glass-blur', '--crm-glass-blur-strong', '--crm-glass-edge'])
  })
})
