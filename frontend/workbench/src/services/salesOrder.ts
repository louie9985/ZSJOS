...
export function salesOrderDetailToListItem(order: SalesOrder): SalesOrderListItem {
  return {
    ...
    studentWechatId: order.studentWechatId,
    provinceName: order.provinceName,
    cityName: order.cityName,
    agreedExamTime: order.agreedExamTime,
    classType: order.classType,
    studentNatureLabelSnapshot: order.studentNatureLabelSnapshot,
    servicePeriodLabelSnapshot: order.servicePeriodLabelSnapshot,
    studentSourceLabelSnapshot: order.studentSourceLabelSnapshot,
    totalAmount: order.totalAmount,
    customerPaidAt: order.customerPaidAt,
    feeModeLabelSnapshot: order.feeModeLabelSnapshot,
    paymentMethodLabelSnapshot: order.paymentMethodLabelSnapshot,
    remark: order.remark,
    studentSpecialRequirements: order.studentSpecialRequirements,
    materialDeliveryContact: order.materialDeliveryContact,
    repurchaseReason: order.repurchaseReason,
    terminationReason: order.terminationReason,
    productSummary: (order.items || []).map(item => `${item.productName} / ${item.skuName}`).join('；'),
    leadNo: order.leadProfile?.leadNo,
    leadSourceLabel: order.leadProfile?.sourceLabel,
    leadSourceUserName: order.leadProfile?.sourceUserName,
    leadOwnerUserName: order.leadProfile?.ownerUserName,
    leadCategoryLabelSnapshot: order.leadProfile?.leadCategoryLabelSnapshot,
    leadSourceChannelLabelSnapshot: order.leadProfile?.sourceChannelLabelSnapshot,
    leadProvinceName: order.leadProfile?.provinceName,
    leadCityName: order.leadProfile?.cityName,
