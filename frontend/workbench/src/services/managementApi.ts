...
export type BusinessAudit = { id: number; operatorUserId?: number; operatorNameSnapshot: string; operatorRoleSnapshot: string; categoryCode: string; actionCode: string; targetType: string; targetId?: string; sourceIp?: string; sourceType: 'ADMIN' | 'PARTNER' | 'PUBLIC_CALLBACK' | 'SYSTEM' | 'EXPLICIT'; traceId?: string; requestMethod?: string; requestPath?: string; resultStatus: 'STARTED' | 'SUCCESS' | 'FAILURE'; resultCode?: number; resultMessage?: string; occurredAt: Timestamp; finishedAt?: Timestamp; durationMs?: number }
...
export type FinanceSource = { leadAccess: 'available' | 'denied' | 'unavailable'; leadId?: number; leadNo?: string; customerName?: string; orderAccess: 'available' | 'denied' | 'unavailable' | 'not_applicable'; orderId?: number; orderNo?: string; studentName?: string; salesName?: string; orderStatusLabel?: string; orderTypeLabel?: string; orderTotalAmount?: number; orderPayableAmount?: number; customerPaidAt?: Timestamp; productName?: string; skuName?: string; quantity?: number; unitPrice?: number; discountAmount?: number; itemPayableAmount?: number }
export type WithdrawalSource = { id: number; amount: number; active: boolean; cashback?: Cashback }
export type CashbackWithdrawal = { id: number; withdrawalNo: string; amount: number; active: boolean; status: string; submittedAt: Timestamp }
export type Cashback = { beneficiaryName?: string; partnerName?: string; source?: FinanceSource; id: number; cashbackNo: string; type: 'valid' | 'deal'; status: 'pending_settlement' | 'available' | 'withdrawing' | 'withdrawn' | 'cancelled'; beneficiaryUserId: number; leadId?: number; leadNo?: string; orderId?: number; orderItemId?: number; productRefSnapshot?: string; productNameSnapshot: string; baseAmount?: number; rateSnapshot?: number; amount: number; observationDaysSnapshot?: number; generatedAt: Timestamp; availableAt?: Timestamp; settledAt?: Timestamp; cancelledAt?: Timestamp; cancelReason?: string }
export type Withdrawal = { applicantName?: string; partnerName?: string; paidByName?: string; cashbackCount?: number; version?: number; reviewedByName?: string; reviewReason?: string; availableActions?: string[]; reviewUnavailableReason?: string; id: number; withdrawalNo: string; applicantUserId: number; status: string; verificationStatus: string; applicationAmount: number; availableBalanceSnapshot?: number; accountNameSnapshot: string; maskedCardNumber: string; cardNumber?: string; bankNameSnapshot: string; branchNameSnapshot?: string; submittedAt: Timestamp; approvedAmount?: number; reviewedByUserId?: number; reviewedAt?: Timestamp; rejectionReason?: string; bankTransactionNo?: string; proofUrl?: string; payoutRemark?: string; paidByUserId?: number; paidAt?: Timestamp; items?: Array<{ cashbackId: number; amount: number }> }
export type WithdrawalPayoutRequest = { paidAt?: string; remark?: string }
export type RelationScene = { id?: number; name: string; code: string; sourceLabel: string; targetLabel: string; sourceType?: 'system_user' | 'partner'; sourcePostCode?: string; targetPostCode?: string; sourcePostCodes?: string[]; targetPostCodes?: string[]; targetEligibilityType: 'post' | 'permission'; targetPermissionCode?: string; status: number; remark?: string }
...
export const managementApi = {
  ...
  businessAudits: async (params: { pageNo: number; pageSize: number; categoryCode?: string; actionCode?: string; targetType?: string; sourceType?: string; resultStatus?: string; operatorUserId?: number }) => unwrap<PageResult<BusinessAudit>>(await http.get('/zsjos/business-audit/page', { params })),
  ...
  cashback: async (id: number) => unwrap<Cashback>(await http.get(`/zsjos/cashback/${id}`)),
  cashbackWithdrawals: async (id: number, pageNo = 1) => unwrap<PageResult<CashbackWithdrawal>>(await http.get(`/zsjos/cashback/${id}/withdrawals`, { params: { pageNo, pageSize: 10 } })),
  withdrawalSources: async (id: number, pageNo = 1) => unwrap<PageResult<WithdrawalSource>>(await http.get(`/zsjos/withdrawal/${id}/sources`, { params: { pageNo, pageSize: 10 } })),
  cashbacks: async (own: boolean, params: { pageNo: number; pageSize: number; type?: string; status?: string; keyword?: string; advancedFilter?: import('./api').AdvancedFilterGroup }) => params.advancedFilter ? unwrap<PageResult<Cashback>>(await http.post(`/zsjos/cashback/${own ? 'my-' : ''}search-page`, params)) : unwrap<PageResult<Cashback>>(await http.get(`/zsjos/cashback/${own ? 'my-' : ''}page`, { params })),
  withdrawals: async (own: boolean, params: { readScope?: 'SELF' | 'ALL' | 'USER'; targetUserId?: number; pageNo: number; pageSize: number; status?: string; keyword?: string; advancedFilter?: import('./api').AdvancedFilterGroup }) => params.advancedFilter ? unwrap<PageResult<Withdrawal>>(await http.post(`/zsjos/withdrawal/${own ? 'my-' : ''}search-page`, params)) : unwrap<PageResult<Withdrawal>>(await http.get(`/zsjos/withdrawal/${own ? 'my-' : ''}page`, { params })),
  ...
  reviewWithdrawal: async (id: number, approve: boolean, data: { version: number; reason?: string }) => unwrap<boolean>(await http.put(`/zsjos/withdrawal/${id}/${approve ? 'approve' : 'reject'}`, data)),
  ...
  payoutWithdrawal: async (id: number, data: WithdrawalPayoutRequest) => unwrap<boolean>(await http.put(`/zsjos/withdrawal/${id}/payout`, data)),
  batchPayoutWithdrawals: async (data: WithdrawalPayoutRequest & { ids: number[] }) => unwrap<boolean>(await http.put('/zsjos/withdrawal/batch-payout', data)),
