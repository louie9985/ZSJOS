// FMS 领域常量。会计科目类型/借贷方向等是会计准则固定的技术枚举（根 AGENTS.md 字典契约的豁免项）。
// TODO: FMS_ACCOUNTING_STANDARD_OPTIONS 与 fms_accounting_standard 字典重复，系 admin 既有不一致，照搬不改。
...
export const FMS_CLOSING_VOUCHER_TYPE_OPTIONS = [
  { label: '收益和损失分开结转（分别生成收益凭证和损失凭证）', value: FMS_CLOSING_VOUCHER_TYPE.SEPARATE_GAIN_AND_LOSS },
  { label: '收益和损失同时结转', value: FMS_CLOSING_VOUCHER_TYPE.COMBINED_GAIN_AND_LOSS }
] as const
