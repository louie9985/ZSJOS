...
const retryAfterRefresh = async (config: AxiosRequestConfig, originalError: unknown) => {
  ...
  const sentAuthorization = String(request.headers?.Authorization || request.headers?.authorization || '')
  if (sharedAccessToken && sentAuthorization !== `Bearer ${sharedAccessToken}`) {
    request.headers = { ...request.headers, Authorization: `Bearer ${sharedAccessToken}` }
    return http(request)
  }
  if (!refreshing) {
