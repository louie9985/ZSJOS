...
经确认的例外：媒体学员定位访谈概览使用既有客户档案字段块样式承载学员及访谈信息，并由既有自适应操作栏统一收集服务端授权的学习规划、交付和编导操作。
该例外仅适用于“概览”标签页内容。左侧学员列表、搜索栏、头像与姓名详情头和标签页外框沿用原有组件及样式，不随访谈内容调整而重做。
...
账号页只保留截图内容，移除改版前维护历史及独立定位卡、内容生产、拍剪区块和操作入口。新档案追加记录保留；后端历史数据及其他业务页面不因展示收口而删除。责任使用红/蓝/黄标签、文字和左色条，自动及非本人字段只读，全部颜色引用 crm token；主页图责任未配置时显示待确认。
