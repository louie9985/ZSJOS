...
public class FileServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testPresignGetUrlsUsesEachFilesOriginalConfig() {
        FileDO first = randomPojo(FileDO.class, o -> o.setConfigId(10L).setPath("lead/first.jpg"));
        FileDO second = randomPojo(FileDO.class, o -> o.setConfigId(20L).setPath("lead/second.jpg"));
        fileMapper.insert(first);
        fileMapper.insert(second);
        FileClient firstClient = mock(FileClient.class);
        FileClient secondClient = mock(FileClient.class);
        when(fileConfigService.getFileClient(10L)).thenReturn(firstClient);
        when(fileConfigService.getFileClient(20L)).thenReturn(secondClient);
        when(firstClient.presignGetUrl("lead/first.jpg", 600)).thenReturn("https://signed.test/first");
        when(secondClient.presignGetUrl("lead/second.jpg", 600)).thenReturn("https://signed.test/second");

        Map<Long, String> result = fileService.presignGetUrls(List.of(first.getId(), second.getId()), 600);

        assertEquals("https://signed.test/first", result.get(first.getId()));
        assertEquals("https://signed.test/second", result.get(second.getId()));
    }
