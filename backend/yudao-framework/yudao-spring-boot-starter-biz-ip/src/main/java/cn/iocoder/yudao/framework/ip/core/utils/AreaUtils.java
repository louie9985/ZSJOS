...
public class AreaUtils {
     ...
     * Area 内存缓存，提升访问速度
     */
    private static volatile Map<Integer, Area> areas;
    private static volatile AreaCommonApi areaApi;
    private static volatile long databaseLoadedAt;
    private static final long DATABASE_CACHE_MILLIS = 60_000L;
     ...
     * 初始化
     */
    private static void initFromCsv() {
        try {
    ...
    /**
     * Registers the System database provider while retaining CSV as a fallback for standalone runtimes.
     */
    public static void init(AreaCommonApi api) {
        areaApi = api;
        clearCache();
        log.info("[init][AreaUtils switched to System area data]");
    }
    ...
    private static Map<Integer, Area> currentAreas() {
        AreaCommonApi api = areaApi;
        if (api == null) {
            return areas;
        }
        long now = System.currentTimeMillis();
        if (databaseLoadedAt > 0 && now - databaseLoadedAt < DATABASE_CACHE_MILLIS) {
            return areas;
        }
        synchronized (AreaUtils.class) {
            ...
            List<AreaCommonRespDTO> records = api.getAreaList();
            Map<Integer, Area> loaded = new HashMap<>();
            Map<Integer, Integer> sorts = new HashMap<>();
            loaded.put(Area.ID_GLOBAL, new Area(Area.ID_GLOBAL, "全球", 0, null, new ArrayList<>()));
            for (AreaCommonRespDTO record : records) {
                loaded.put(record.getId(), new Area(record.getId(), record.getName(), record.getType(), null, new ArrayList<>()));
                sorts.put(record.getId(), record.getSort() == null ? 0 : record.getSort());
            }
            for (AreaCommonRespDTO record : records) {
                Area area = loaded.get(record.getId());
                Area parent = loaded.get(record.getParentId());
            ...
            loaded.values().forEach(area -> area.getChildren().sort(
                    Comparator.comparing((Area child) -> sorts.getOrDefault(child.getId(), 0))
                            .thenComparing(Area::getId)));
            areas = loaded;
    ...
    public static Area parseArea(String pathStr) {
        ...
        for (String path : paths) {
            if (area == null) {
                area = findFirst(currentAreas().values(), item -> item.getName().equals(path));
            } else {
    ...
    public static <T> List<T> getByType(AreaTypeEnum type, Function<Area, T> func) {
        return convertList(currentAreas().values(), func, area -> type.getType().equals(area.getType()));
    }
