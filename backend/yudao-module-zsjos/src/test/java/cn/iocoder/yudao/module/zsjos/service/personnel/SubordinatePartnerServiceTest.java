...
class SubordinatePartnerServiceTest {
    ...
    void outOfRangePageReturnsCountWithoutLoadingRows() {
        ...
        when(ownershipService.getReadableEmployeeUserIds(20L)).thenReturn(Set.of(20L, 21L));
        when(ownershipMapper.selectScopedCount(9L, Set.of(20L, 21L), null, "张三")).thenReturn(20L);

        var result = service.getPage(request(Integer.MAX_VALUE, "  张三  "), 20L);

        assertTrue(result.getList().isEmpty());
        assertEquals(20L, result.getTotal());
        verify(ownershipMapper, never()).selectScopedPage(anyLong(), anyCollection(), any(), any(), anyLong(), anyInt());
    }
    ...
    void pageUsesLongOffsetAndMapsOwnershipProjection() {
        when(ownershipService.canQuery(20L)).thenReturn(true);
        when(ownershipService.getReadableEmployeeUserIds(20L)).thenReturn(Set.of(20L, 21L));
        when(ownershipMapper.selectScopedCount(9L, Set.of(20L, 21L), null, null)).thenReturn(41L);
        ...
        when(ownershipMapper.selectScopedPage(9L, Set.of(20L, 21L), null, null, 40L, 20)).thenReturn(List.of(row));
