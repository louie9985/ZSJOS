...
public class DeliveryClassController {
    ...
    @GetMapping("/{id}") @PreAuthorize("@ss.hasAnyPermissions('zsjos:delivery-class:query-managed','zsjos:delivery-class:query-my')")
    public CommonResult<DeliveryClassRespVO> get(@PathVariable Long id){ return success(service.get(id,getLoginUserId())); }
    @GetMapping("/{id}/students") @PreAuthorize("@ss.hasAnyPermissions('zsjos:delivery-class:query-managed','zsjos:delivery-class:query-my')")
