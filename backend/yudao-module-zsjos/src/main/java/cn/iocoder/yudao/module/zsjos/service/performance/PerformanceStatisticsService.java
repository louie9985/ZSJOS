...
public class PerformanceStatisticsService {
 ...
 public Analysis analysis(Query q){
  ...
  return new Analysis(now(),w.start().toLocalDate(),q.getEnd()==null?now().toLocalDate():q.getEnd(),averages,trend,groups(selected,x->Objects.toString(x.getChannelCode(),"unknown")+"|"+Objects.toString(x.getLabel(),"历史来源缺失"),false),groups(products,x->Objects.toString(x.getGroupKey(),"unknown")+"|"+Objects.toString(x.getLabel(),"历史产品名称缺失"),true),groups(selected,x->Objects.toString(x.getUserId(),"unknown")+"|"+Objects.toString(x.getUserName(),"历史姓名缺失"),false),contributions,averageTrends,targetPeriod(q,w)==null?null:targets.resolve(q.getScopeType(),q.getScopeId(),targetPeriod(q,w),w.start().toLocalDate()));
 }
