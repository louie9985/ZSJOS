...
public class PublicPaymentController {
    private static final Long PUBLIC_TENANT_ID = 1L;
    ...
    public CommonResult<PublicPaymentDetailRespVO> detail(@PathVariable String no, @RequestParam @NotBlank String token) {
        return success(TenantUtils.execute(PUBLIC_TENANT_ID, () -> service.publicDetail(no, token)));
    }
    ...
    public CommonResult<Object> order(@PathVariable String no, @RequestBody Map<String, String> request) {
        Object data = TenantUtils.execute(PUBLIC_TENANT_ID,
                () -> service.publicOrder(no, request.get("token"), request.get("channel")));
        return success(data);
    }
    ...
    public ResponseEntity<String> wechatOrder(@PathVariable String no, @RequestParam @NotBlank String token,
        ...
        return ResponseEntity.ok().contentType(MediaType.TEXT_HTML)
                .body(TenantUtils.execute(PUBLIC_TENANT_ID, () -> service.publicWechatOrderHtml(no, token)));
    }

    @PostMapping("/{no}/status")
    public CommonResult<Boolean> status(@PathVariable String no, @RequestBody Map<String, String> request) {
        return success(TenantUtils.execute(PUBLIC_TENANT_ID,
                () -> service.publicStatus(no, request.get("token"))));
    }
}
