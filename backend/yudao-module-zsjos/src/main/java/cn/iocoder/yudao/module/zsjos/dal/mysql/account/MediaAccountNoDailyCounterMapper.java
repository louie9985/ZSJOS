...
import cn.iocoder.yudao.module.zsjos.dal.dataobject.account.MediaAccountNoDailyCounterDO;
...
@Mapper
public interface MediaAccountNoDailyCounterMapper extends BaseMapperX<MediaAccountNoDailyCounterDO> {
     ...
     */
    @Insert("""
            INSERT IGNORE INTO zsjos_media_account_no_daily_counter
    ...
    @Update("""
            UPDATE zsjos_media_account_no_daily_counter
               SET current_value = IF(current_value >= #{maxSequence}, 1, current_value + 1),
                   update_time = NOW()
             WHERE tenant_id = #{tenantId} AND sequence_date = #{sequenceDate} AND deleted = b'0'
            """)
    ...
    @Select("""
            ...
            FROM zsjos_media_account_no_daily_counter
            WHERE tenant_id = #{tenantId} AND sequence_date = #{sequenceDate} AND deleted = b'0'
            FOR UPDATE
            """)
