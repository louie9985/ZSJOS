...
import cn.iocoder.yudao.module.zsjos.service.audit.*;
...
@Component @Slf4j public class EmployeeContractAnniversaryReminderJob implements JobHandler {
    ...
    @Resource private HrmEmployeeReminderApi api; @Resource private BusinessTaskCommandService tasks; @Resource private NotifyBusinessEventApi notify; @Resource private BusinessAuditService auditService;
    @Override @TenantJob public String execute(String param){ LocalDateTime now=LocalDateTime.now(ZONE); int count=process(api.getDueContractExpiryDispatch(now.toLocalDate(),now.toLocalTime()), true, now); count+=process(api.getDueEntryAnniversaryDispatch(now.toLocalDate(),now.toLocalTime()), false, now); auditService.recordExecution(AuditActionCatalog.EXECUTION_QUARTZ, "employee-contract-anniversary-reminder", Map.of("processed", count)); return "员工合同/周年提醒：处理 "+count+" 条"; }
