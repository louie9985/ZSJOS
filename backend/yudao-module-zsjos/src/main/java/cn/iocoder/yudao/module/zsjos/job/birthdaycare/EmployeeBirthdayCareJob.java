...
import cn.iocoder.yudao.module.hrm.api.employeereminder.HrmEmployeeReminderDispatch;
import cn.iocoder.yudao.module.hrm.api.employeereminder.HrmEmployeeReminderEmployee;
...
public class EmployeeBirthdayCareJob implements JobHandler {
    ...
    public String execute(String param) {
        ...
        HrmEmployeeReminderDispatch dispatch = birthdayCareApi.getDueBirthdayDispatch(now.toLocalDate(), now.toLocalTime());
        if (dispatch.isEmpty()) return "生日关怀：无待处理员工或接收人";
        int created = 0;
        for (HrmEmployeeReminderEmployee employee : dispatch.employees()) {
            for (Long recipientUserId : dispatch.recipientUserIds()) {
                String key = "hrm-birthday-care:" + dispatch.targetDate().getYear() + ":"
                        + employee.id() + ":" + recipientUserId;
                try {
                    String summary = (employee.deptName() == null ? "" : employee.deptName() + " · ")
                            + employee.targetDate().format(MONTH_DAY);
                    ...
                    payload.put("birthday", employee.targetDate().format(MONTH_DAY));
