...
public class AdvancedFilterService {
    ...
    private Object typed(String type, Object raw) {
        ...
        try {
            return switch (type) {
                ...
                case "date" -> LocalDateTime.parse(raw.toString());
                default -> raw.toString().trim();
            };
        } catch (NumberFormatException | DateTimeParseException ex) { throw exception(ADVANCED_FILTER_INVALID); }
    }
    ...
    private static Map<String, Field> fields() {
        ...
        add.accept(select("person.identityStatus", "客户", "身份状态", "p.identity_status", null));
        ...
        add.accept(select("lead.status", "客资", "客资状态", "l.status", "lead-status"));
        add.accept(select("lead.assignmentStatus", "客资", "分配状态", "l.assignment_status", "lead-assignment-status"));
        ...
        add.accept(select("opportunity.status", "商机", "商机状态", "op.status", "opportunity-status"));
        ...
        add.accept(select("order.status", "订单", "订单状态", "so.status", "order-status"));
        add.accept(select("order.type", "订单", "订单类型", "so.order_type", "order-type"));
