...
public class WithdrawalNotifySceneProvider implements NotifySceneProvider {
    ...
    @Override public Set<Long> resolveRecipients(NotifyBusinessEvent event, Set<String> roles) {
        Set<Long> result = new LinkedHashSet<>(); Map<String, Object> payload = event.getPayload();
        if (roles.contains(ROLE_APPLICANT) && payload.get("applicantUserId") instanceof Number n) result.add(n.longValue());
        if (roles.contains(ROLE_FINANCE) && payload.get("financeUserIds") instanceof Collection<?> ids) {
            ids.stream().filter(Number.class::isInstance).map(Number.class::cast).map(Number::longValue).forEach(result::add);
        }
        return result;
    }
    @Override public Map<String, Object> resolveVariables(NotifyBusinessEvent event, Long recipientUserId) {
