...
public class NotifyTemplateServiceImplTest extends BaseDbUnitTest {
    ...
    public void testUpdateNotifyTemplate_notExists() {
        // 准备参数
        NotifyTemplateSaveReqVO reqVO = randomPojo(NotifyTemplateSaveReqVO.class, o -> o.setSceneCode(null));
    ...
    public void testFormatNotifyTemplateContent() {
        ...
        assertEquals("客资 100 已派给小红",
                notifyTemplateService.formatNotifyTemplateContent("客资 {{lead.id}} 已派给{name}",
                        Map.of("lead.id", 100L, "name", "小红")));
        assertEquals(List.of("lead.id", "name"),
                notifyTemplateService.parseTemplateParams("{{lead.id}}", "摘要{name}", "{{lead.id}}"));
    }
}
