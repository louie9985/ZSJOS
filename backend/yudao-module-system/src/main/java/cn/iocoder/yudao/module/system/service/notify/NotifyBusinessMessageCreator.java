...
public class NotifyBusinessMessageCreator {
    ...
    public void create(NotifyBusinessEvent event, NotifySceneProvider provider, NotifyRuleDO rule,
                       NotifyTemplateDO template, Long recipientId) {
        if (notifyMessageService.existsByRuleUserAndEvent(rule.getId(), recipientId, event.getSourceEventKey())) {
            return;
        }
        Map<String, Object> variables = provider.resolveVariables(event, recipientId);
        ...
        notifyMessageService.createNotifyMessage(NotifyMessageCreateReqDTO.builder()
                .userId(recipientId).userType(UserTypeEnum.ADMIN.getValue()).template(template)
