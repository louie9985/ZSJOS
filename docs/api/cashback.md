...
The independent partner frontend uses `GET /app-api/zsjos/cashback/my-page` and `GET /app-api/zsjos/cashback/my-summary`; both require `zsjos:cashback:my-query` and restrict rows to the current beneficiary. `GET /admin-api/zsjos/cashback/page` remains the finance-wide administrator view and requires `zsjos:cashback:finance-query`.
