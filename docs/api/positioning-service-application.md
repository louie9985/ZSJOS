...
工作台学员概览的“修订定位卡”和定位卡列表的“修改定位卡”均先弹出二次确认，默认聚焦“取消”。弹窗明确提示：确认后主卡进入草稿，修改后需重新提交运营审核、学员确认并补齐确认凭证，历史版本和账号已应用版本保留。只有点击“确认修改”才调用 start-revision；取消不发起修订请求、不改变状态或打开编辑器。草稿的“继续填写／提交审核”沿用直接编辑入口。
...
中世健员工工作台打开已有主卡时，以该卡 templateId 查询该模板最新发布版，不能使用当前默认模板替代原卡模板。默认模板仅用于新建；同一模板内退役字段仍按既有契约保留在服务端快照而不展示，不将其伪装为新字段。

资料预审每次打开均重新读取课程服务联系上下文和草稿版本，读取失败显示错误、不打开空白编辑器，避免自动保存后重开复用旧预约时间和旧版本号。
