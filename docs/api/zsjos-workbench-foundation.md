...
The owner Lead detail includes a lazy-loaded read-only `订单记录（数量）` tab. It aggregates all first-purchase and repurchase orders by the Lead's Person, uses a desktop master-detail layout and a mobile order selector, caches full details by order ID for the current Lead, and clears the cache when the Lead changes.
