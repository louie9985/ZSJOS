...
- 只返回 `yudao.media-screen.new-media.department-ids` 明确配置的根部门，顺序与配置一致；每个根部门的全部下级部门自动纳入该根部门的统计与人员范围。
- 实时成员名单来自 System 当前组织：根部门及其全部下级部门中的所有启用用户均展示，零贡献显示四项 `0`；不按角色、岗位或历史贡献反推名单。跨模块花名册查询不继承公共请求的数据权限。
- 所有统计只读取 Lead 首次计数时冻结的 `countedAt`、规范提供方和贡献员工/部门/主管快照，不按当前组织或 Partner 归属反推历史。例外是已明确标记为 `legacy-parttimecrm-*` 的导入兼职客资：V266 仅在其贡献员工和部门快照均为空且存在当前有效兼职归属时，按该确认归属一次性回填快照；常规业务数据不会追溯改派。
...
- 汇总保留停用员工及已调离员工的历史贡献，实时成员榜只展示当前仍在配置部门树内的启用员工；实时 Partner 明细过滤当前停用 Partner，但员工兼职汇总保留其历史贡献。
- 实时陪跑明细按当前兼职归属补齐启用兼职：统计窗口内无贡献也显示今日、本周、月总、有效四项 0；当前范围内的启用运营即使没有贡献行，只要名下有启用兼职也展示。已有贡献行继续使用原归属快照和名称，补零不转移或重复累计历史业绩。历史页面仍仅展示已冻结名单。
- 员工跨配置根部门调动后只在当前部门展示，个人卡片只承接冻结到当前根部门的贡献；调动前贡献仍留在原部门汇总，因此该场景下部门汇总允许大于当前可见成员明细之和。
...
- V266 不改写既有 `zsjos_media_screen_daily_snapshot`，因此已冻结的历史日期页面、昨日冠军仍以原快照为准；实时统计、趋势和近 14 日序列在缓存刷新后使用回填后的 Lead 快照。
