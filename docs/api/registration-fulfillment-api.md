...
- Full student read does not replace individual account/positioning/content feature and object permissions. Interview context and bound attachment downloads use `positioning-interview-read`, separate from `director-interview` commands. Full readers may inspect historical drafts; available actions remain constrained to the active, accepted service's actual director. Legacy talk creation independently requires personal service participation, even when student reads are unrestricted.
- V270 adds the configurable full-read button under the existing media-student page without assigning roles or altering tenant packages. Vue Admin does not render this Workbench-owned page. No authentication or response-envelope contract changes are introduced.
- When historical assets remain without a course relationship, Workbench renders the retained accounts and a read-only positioning-draft entry instead of hiding the entire detail. Opening a draft uses the existing card read API; no service relationship or command is synthesized. Read failures remain visible with retry.
...
Workbench 指派弹窗只读展示当前学员（学员详情姓名）、学员编号（`personNo`，缺失显示“历史未记录”）和当前运营（当前服务上下文的 `operatorUserName`）；未指派时显示“未指派”，已有运营但姓名缺失时显示“姓名未提供”。选择新的运营负责人不会改变当前运营展示。弹窗始终提供原因输入；普通更换和冲突时前置校验，服务端要求补充原因时保留表单并显示字段错误。每次打开重新读取上下文和候选，加载失败可重试；版本冲突及候选失效需刷新并重新确认。服务关系、未归档定位卡及学员账号的运营同步沿用原事务，记录分配原因并发送既有通知；不改写历史快照。成功后独立刷新列表和当前课程详情。
