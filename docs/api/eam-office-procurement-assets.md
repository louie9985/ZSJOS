...
- 单件候选只来自同分类、同规格自定义字段签名（排除单台标识 `sn`）的闲置资产卡，并排除已预留和仍有持有记录的
...
全部留空，填写时必须与本次数量一致且不能重复。序列号入口取决于分类是否配置自定义字段 `sn`；IT 根分类配置后由子分类继承，逐台保存至资产 `extFields.sn`，不再写固定列。未配置该字段的单件分类按系统资产编号入退库。查重与退货序列号定位读取自定义字段，退货仍校验原入库来源。单台序列号不参与库存规格匹配；入库金额仍记采购/收货单，不再复制为资产原值或净值。批量入库增加库存余额和流水，定向且需归还
的数量同时生成待签收持有记录。
