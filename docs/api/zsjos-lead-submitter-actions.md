...
- `PUT /part-api/zsjos/lead/{id}/submitter-supplement` (and the existing ADMIN equivalent) appends one required supplement remark and up to nine JPG/PNG/WebP image attachments. It never changes submitted Lead attributes or the existing Lead remark. Legacy region/category/product fields may be sent during rolling deployment but are ignored.
...
- Supplement events remain `lead_submitter_supplemented`. Their typed `relatedObjectRefs` snapshot retains the trimmed remark, typed submitter identity, name snapshot and SHA-256 digest of normalized remark plus ordered attachment IDs; `evidenceRefs` stores the attachment metadata snapshot. Writes, activity timestamp and event insert are atomic. Same-key matching requests return success; mismatched replay returns error `1900003130`.
...
- Detail `remarkHistory` entries now include the attachments belonging to that entry. New supplements sort newest first; the initial submission remains last. New clients use the compatibility `remark` only when `remarkHistory` is absent, not when it is an empty array. Existing list/filter consumers remain a single stored field, not an aggregate of new supplements.
