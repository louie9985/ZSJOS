...
- `PUT /zsjos/sales-order/{id}/resubmit`：修改 `revision_required` 或 `terminated` 原订单并创建新审批轮次。
...
- `terminated`：创建人或正式负责人主动终止未生效订单；当前 BPM 轮次同时终止，可由有权人员修改原订单并创建新轮次重提。首购恢复商机跟进状态，复购不修改客资或商机。
