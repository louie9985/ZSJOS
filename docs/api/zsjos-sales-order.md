...
公海池中的协同销售只能跟进并申请正式转派，或由主管直接转派；服务端拒绝其直接提交首购订单，不执行隐式转权。正式转派成功后退出公海、更新 Lead 与 Opportunity 正式负责人、记录转派历史并换绑未完成跟进任务，之后新负责人才能录入成交。订单 `formalSalesUserId` 固化正式负责人，`submitterUserId` 记录实际操作人；后续驳回、取消或终止不回滚已完成的正式转派。正式负责人在公海中提交时仍可录入，周期进入 `deal_pending`。
