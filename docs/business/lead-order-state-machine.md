...
报名履约与财务继续并行会签。上线后新轮次中，任一中心普通审批人可在本中心首次申请直属主管确认；申请、确认和不确认意见均必填且不超过 1000 字。
...
- 主管固定取申请人直属部门 `leaderUserId`，必须启用且不同于申请人；每轮每中心唯一记录阻止再次申请。
- 申请中心锁定期间，该中心所有普通通过/驳回命令均失败；另一中心不受影响。
- 申请与决定进入订单命令账本并递增订单、轮次版本，主管记录另有版本；订单和轮次行锁保证并发命令只有首个有效。
- 新轮次普通审批池排除报名/财务根部门及其子部门的所有部门负责人。排除后任一中心为空时拒绝提交。
