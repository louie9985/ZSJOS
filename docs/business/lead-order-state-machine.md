...
判定有效在同一事务内完成判定任务、保存必填有效备注、创建唯一 `initial_conversion` Opportunity，并将 Lead 改为 `converted + closed`。之后的跟进写入 Opportunity 跟进记录，并维护机会状态和提醒；判无效会同时取消待处理的首跟、判定和跟进提醒任务，并把未结束 Opportunity 改为 `lost`。任务查询还会排除无效客资遗留的上述待处理任务；`V033` 负责取消规则上线前的历史遗留记录。无效 Lead 仍允许当前负责人追加证据型跟进，但不创建首跟、判定或提醒任务。
