...
6. 销售从全部启用 SKU 中选择成交课程，填写实际成交金额、付款时间、缴费方式、支付方式和缴费凭证后直接提交会签；订单固化客户、产品、金额和文件快照。
...
14. `registrationReview` 首次通过后即幂等创建报名服务单，并发布新报名履约任务通知；订单生效前或财务待补正时可编辑清单，但只有订单 `effective`、清单全选且规划师有效时才可完成。完成后将 Person 转为学员并按订单明细建立服务关系。
...
补正任务仅对应明确业务驳回，包括报名履约驳回、财务驳回和主管不确认导致整轮驳回。任务类型为 `sales_order_revision`，无截止时间，只派给本轮提交人；正式负责人代为重提成功也完成该任务。BPM 异常取消或异常终止不创建任务，功能上线前的历史 `revision_required` 订单不回填。当前生产代码尚不存在把订单正式转换为 `superseded` 的业务操作，因此补正任务取消必须在未来该真实状态转换事务中接入，不能仅凭常量推测并提前制造状态入口。
...
报名履约与财务继续并行会签。上线后新轮次中，任一中心普通审批人可在本中心首次申请订单销售主管确认；申请、确认和不确认意见均必填且不超过 1000 字。

```text
中心普通审批中 --申请主管确认--> 中心锁定 / 主管向前加签
中心锁定 --主管确认--> 恢复本中心全部普通审批人
中心锁定 --主管不确认--> BPM 驳回整轮 -> revision_required
中心锁定 --另一中心驳回/销售终止/流程取消--> confirmation.cancelled
```

- 主管固定取订单正式销售当前直属部门 `leaderUserId`，必须启用、不同于正式销售本人且持有主管确认权限；每轮每中心唯一记录阻止再次申请。
- 申请中心锁定期间，该中心所有普通通过/驳回命令均失败；另一中心不受影响。
