...
客资销售反馈是主状态之外的追加记录，只允许当前销售负责人发送多行文本与附件给
权威提交方；无效、关闭和已成交状态拒绝新增。它不替代跟进记录、成交或审批动作，
也不更新跟进计时。历史反馈保留，通知和详情权限见
[销售反馈契约](../api/lead-submitter-feedback.md)。
...
| `owner_user_id` | bigint | 条件必填 | 当前主责销售；`owned` 时必须存在，`public_pool` 与 `recycle_pending` 时必须为空 |
...
| `recycle_source_owner_user_id` | bigint | 回收待处理时必填 | 回收前销售，仅用于 `recycle_pending` 的主管对象范围校验；进入抢单池时清空 |
...
客资仍为 `submitted` 且已经归属时，销售可以追加 `LeadFollowUpRecord`。归属成立时即创建 `lead_first_follow_up` 和 `lead_qualification` 任务；首次跟进截止时间和有效性判定截止时间均从当前归属成立时间计算，分别固化当时启用规则的编号、版本、时限及截止时间，后续规则修改不追溯已有轮次。新增跟进记录不改变 Lead 主状态；当前归属周期首次记录完成 `lead_first_follow_up`，但不重新创建或推迟 `lead_qualification`。首次跟进完成前处理阶段为待首跟，但仍展示有效性判定截止时间；首跟逾期只提醒，判定超时扫描将客资挂起。可选的下次跟进时间创建或替换 `lead_follow_up_reminder`。记录只追加，方式、结果和分类标签均固化快照。
...
| 首次跟进 | `submitted + owned` | 主状态不变 | 记录本周期首跟事实并完成已有首跟任务；不重建或延长判定轮次，迟到首跟仍允许提交 |
...
| 释放到抢单池 | `suspended + owned` / `recycle_pending` | `submitted + public_pool` | 清空当前负责人及当前归属周期字段，原归属仅保留在分配历史；被抢后重新进入待首跟 |
...
一般派单、接单、拒单、公海释放和认领只改变 `assignment_status`、负责人和分配历史；形成归属时同时启动首跟和有效性判定计时。判定超时挂起是新增例外：扫描把 `Lead.status` 从 `submitted` 改为 `suspended`；恢复、转派、回收或释放再将主状态恢复为 `submitted`。系统不再因无进展预警或宽限期自动释放到抢单池，抢单池进入仅由明确分配或主管处置动作触发。无效申诉由 `LeadAppeal.status` 表达，不增加 `Lead.status.appealing`。
...
- 待首跟：`status = submitted`、`assignment_status = owned` 且当前归属周期没有首次跟进，`current_assignment_first_follow_up_at IS NULL`；此时有效性判定截止时间已经存在。
...
提交页联系方式查重的同字段和交叉字段均视为强重复；分别命中多个客资时逐条激活和通知，同一请求对同一客资幂等处理。激活不改变历史客资状态或归属，无负责人也保留激活记录并使用统一页面提示。
...
- `assigned` 时 A 与 B 均可新增跟进、设置下次跟进；活动公海期间 A 与 B 均不可录入或补正成交，B 必须先申请或由主管直接完成正式转派；主管退出公海后恢复正常成交资格。
...
- 协同本身不改变正式归属；活动公海期间任何销售不得直接提交首购订单、补正或接续重提，必须先完成正式转派或由主管退出公海。正式转派完成后退出公海并由新负责人录入成交。
...
- 补正权限只属于原创建人或正式负责人；活动公海期间即使是原负责人也不能新建或补正成交。公海跟进销售须先完成正式转派或退出公海后，才按正式负责人权限成交。
...
`revision_required/terminated` 重提成功后，旧订单进入 `superseded`，购买链路指向新的 `pending_approval` 订单。新订单重新创建 BPM 实例和审批快照，不继承上一轮教务或财务通过结论。线下允许纠正课程、SKU 和金额录入值，线上保留原支付交易约束；详细校验、错误码和支付继承以[支付实施契约](./payment-module-development.md)及[订单 API](../api/zsjos-sales-order.md)为准。事务失败时原订单、购买链路、支付关联及分配一起回滚。


### 迁移缺失首跟任务的恢复（2026-09-23）

当前归属周期的首跟事实来自该周期、当前负责人提交的跟进记录，不以任务 UPDATE 的影响行数为依据。跟进保存继续持有租户内 Lead 行锁；已记录的首跟时间保持不变，缺失时按本周期本人最早记录的发生时间与 ID 确定首条并标记 `firstInAssignment`。任务不存在、已完成或已取消不阻止首跟事实落库；已有待处理任务同步完成。合作跟进不能完成当前负责人的首跟。已判有效的 Opportunity 跟进路径同样不依赖任务更新返回值。
跟进状态以首跟事实为准；主管恢复/转派直接进入判定且没有新首跟截止时间的既有例外保留。新增跟进不会自动补造归属时间或重置判定时限。
本次历史修复是经确认的显式运维操作：完全缺失判定轮次者从修复时刻按当前规则新建轮次，完整轮次不动；不能将迁移写入的 `other` 归属历史时间当作真实接单时间。详见 `script/sql/mysql/repairs/lead-first-follow-recovery.md`。
