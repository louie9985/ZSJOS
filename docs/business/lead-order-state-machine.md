...
销售直接提交成交订单；订单总额由订单项实际成交金额汇总，非零订单必须上传缴费凭证。订单应展示本次提交的完整信息。模型不增加“报名审核字段清单”或“财务审核字段清单”。财务点击通过表示对销售提交的付款、金额、产品和凭证信息完成复核，不产生支付渠道到账事实。
...
- 订单审批、驳回和创建人终止均锁定订单与当前审批轮次，并登记绑定操作人、节点、BPM 任务和请求指纹的命令账本。相同命令可重放，不同命令复用幂等键必须冲突，BPM 失败整体回滚。
