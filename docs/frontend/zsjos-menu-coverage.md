...
| 55 | 素材类型与模板 | `/zsjos/material-library/types` | `admin_embed` | `zsjos/materialType/index` |
| 56 | 素材导入记录 | `/zsjos/material-library/imports` | `admin_embed` | `zsjos/materialImport/index` |
...
- V270 adds `zsjos:media-student:query-all` as a child button, not a second page. Full readers default to tenant media data, including historical services and drafts; personal readers keep the existing relationship scope. The Workbench heading uses “学员管理” for the configured full permission and interview drafts render read-only when no write action is returned. Vue Admin continues filtering the Workbench-only component.
