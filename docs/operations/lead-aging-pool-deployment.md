...
Migration: `script/sql/mysql/migrations/V033__lead_aging_collaboration_pool.sql`.
...
3. Review the V033 final read-only query. Leads returned by it have no recoverable formal ownership
...
V033 adds the Lead ownership-start and timeout columns, order continuation links, the Lead scan and
...
1. Apply V033 once through the approved migration process. Do not edit an already applied V033.
2. Re-run V033 to verify guarded DDL and seed idempotency, then run `verify-bootstrap.sql`.
3. Grant `manage` and `manage-all` only through the normal System role/menu workflow. V033 copies
