...
本功能不需要数据库迁移，也不会写入业务数据。部署会更换后端制品并需要重启服务；对共享或生产环境执行这些操作前必须另行确认目标实例、时间窗口和回滚责任人。
...
仓库的 `application-local.yaml` 当前按明确授权为租户 `1` 配置了 `0.0.0.0/0`，用于允许任意 IPv4 客户端联调。该配置只随默认 `local` profile 生效，会使任何能够连接本机 `48080` 端口的 IPv4 客户端读取租户 `1` 的媒体大屏数据，不是部署模板，也不得直接复制到共享、测试或生产环境。部署到其他环境时必须通过外部配置替换租户和 CIDR，并收敛到实际客户端或代理出口地址。
...
1. 确认目标后端制品包含 `MediaScreenController`、`MediaScreenAccessFilter`，并确认配置前缀为 `yudao.media-screen`。
2. 确认目标租户 ID、前端或代理的实际出口 IP、代理链和所需 CIDR，执行双人复核。
3. 在部署平台写入外部配置，先保持 `enabled=false`。
4. 部署新后端制品。共享服务的停止、启动或重启必须在单独授权后执行。
5. 使用关闭状态请求验证接口返回 HTTP 503 且消息为“媒体大屏服务未开启”。
6. 配置完整白名单后设置 `enabled=true`，再次重启或刷新配置。当前实现不承诺动态刷新，默认按需要重启处理。
7. 分别执行正确租户/IP、错误租户、错误 IP、缺少参数以及维护状态检查。
8. 最后通过生产反向代理或 3009 开发代理验证，不能只在后端本机直连验证。
...
优先将 `yudao.media-screen.enabled` 设为 `false` 并重启或刷新目标实例，接口会稳定返回 503。若必须回退制品，恢复上一份已验证 JAR 和对应外部配置。该功能没有数据库迁移或业务写入，因此无需数据回滚；Redis 中的 `zsjos:media-screen:*` 缓存可自然过期，不需要批量删除。
