...
本功能需要按顺序应用 V141 新增每日快照表。实时查询不修改 Lead；定时任务会在快照表写入前一天的冻结统计。
...
- 每日到北京时间 `snapshot.hour:snapshot.minute` 后，扫描任务幂等冻结前一天。首次上线不回填历史，从首次成功冻结日期开始可用。
...
2. 在受控数据库先应用 V141，确认空快照表、唯一键和版本记录存在；迁移不回填 Lead 或历史快照。
