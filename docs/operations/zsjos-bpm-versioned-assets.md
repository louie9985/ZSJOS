...
当前推荐版本统一采用 Simple 设计器格式。原 BPMN 1.0.0 资产作为不可变历史版本保留；转换后的 1.1.0 资产保持 Process Key、任务 Key、候选人集合变量、审批顺序和条件语义不变。成交订单使用 Simple 并行分支，需求反馈通过 `hasDepartmentLeader` 跳过表达式实现可选的部门负责人审批。`eam_asset_transfer` 已有的 1.0.0 Simple 资产继续作为推荐版本。
...
带 `formRef` 的 Simple 资产使用稳定表单标记表达跨环境绑定关系。导入器或管理员必须在目标租户解析该标记并绑定实际 `bpm_form.id`，不能把其他环境的表单 ID 写入版本化资产。目前的标记为 `zsjos-system-form:lead-appeal-review`、`zsjos-system-form:sales-order-dual-approval` 和 `zsjos-system-form:student-contact-extension`。完成绑定后才能发布模型。
