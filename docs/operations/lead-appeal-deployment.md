...
当前推荐资产为 `script/bpm/zsjos_lead_appeal_review/1.0.0/process.bpmn20.xml`，流程 Key 固定为 `zsjos_lead_appeal_review`。发布前必须运行 `python script/bpm/validate_manifest.py` 并核对清单 SHA-256。BPMN 必须由管理员在 BPM 管理页面受控创建模型、载入文件并发布，不启用全局自动部署。
