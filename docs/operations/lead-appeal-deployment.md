...
当前推荐资产为 `script/bpm/zsjos_lead_appeal_review/1.0.0/process.bpmn20.xml`，流程 Key 固定为 `zsjos_lead_appeal_review`。发布前必须运行 `python script/bpm/validate_manifest.py` 并核对清单 SHA-256。fresh bootstrap 会创建未发布模型；已有环境仍由管理员在 BPM 管理页面受控创建模型并载入文件。两种路径都必须由管理员审核和发布，不启用全局自动部署。
...
3. 在 Admin 进入“工作流程 → 流程管理 → 流程模型”。fresh bootstrap 环境复核已有的未发布模型；已有环境新建 BPMN 模型，流程标识填写 `zsjos_lead_appeal_review`，流程名称填写“客资申诉复核”，设为不可见、发起范围为全员，并指定流程管理员。
